...
%DIF DEL vStrCueCoding_v4.tex   Wed Mar  7 11:38:22 2018
%DIF ADD vStrCueCoding_v5.tex   Fri Mar 23 21:53:33 2018
...
\DIFdelbegin \DIFdel{In reinforcement learning, adaptive behavior requires assigning credit to the appropriate reward-predictive stimuli in the environment. }\DIFdelend The nucleus accumbens \DIFaddbegin \DIFadd{(NAc) }\DIFaddend has been shown to be important for
learning from feedback, and biasing and invigorating behavior in
response to \DIFdelbegin \DIFdel{reward-predictive cues. Much work has emphasized nucleus accumbens coding of reward features of reward-predictive stimuli such as }\DIFdelend \DIFaddbegin \DIFadd{outcome-predictive cues. NAc encodes outcome-related cue
features such as the }\DIFaddend magnitude and identity of reward. However, not
much is known \DIFdelbegin \DIFdel{as to }\DIFdelend \DIFaddbegin \DIFadd{about }\DIFaddend how features of cues themselves are encoded.  \DIFdelbegin \DIFdel{To test this we }\DIFdelend \DIFaddbegin \DIFadd{We
}\DIFaddend designed a decision making task where rats \DIFdelbegin \DIFdel{had to learn }\DIFdelend \DIFaddbegin \DIFadd{learned }\DIFaddend multiple sets of
\DIFdelbegin \DIFdel{reward-predictive }\DIFdelend \DIFaddbegin \DIFadd{outcome-predictive }\DIFaddend cues, and recorded single-unit activity in the \DIFdelbegin \DIFdel{nucleus accumbens }\DIFdelend \DIFaddbegin \DIFadd{NAc
}\DIFaddend during performance. We found that coding of various cue features
occurred alongside coding of expected outcome. Furthermore, \DIFdelbegin \DIFdel{we found that }\DIFdelend this
...
revealed. \DIFdelbegin \DIFdel{This finding of persistent coding of }\DIFdelend \DIFaddbegin \DIFadd{Encoding of }\DIFaddend cue features in the \DIFdelbegin \DIFdel{nucleus accumbens could be one way in which the brain maintains }\DIFdelend \DIFaddbegin \DIFadd{NAc may enable contextual
modulation of ongoing behavior, and provide }\DIFaddend an eligibility trace of
outcome-predictive stimuli \DIFdelbegin \DIFdel{to be used }\DIFdelend for updating stimulus-outcome associations
to inform future behavior.

\DIFdelbegin \section*{\DIFdel{Significance Statement (120 words)}}
%DIFAUXCMD
%DIFDELCMD < 
...
\DIFdelbegin \DIFdel{A fundamental problem faced by all reinforcement learning agents is which features and/or actions to credit and blame for particular outcomes \mbox{%DIFAUXCMD
...
. However, successful learning requires not only a RPE but also a trace of the preceding actions and/or cues. For example, if you become ill after eating food X in restaurant A, depending on if you remember the identity of the restaurant or the food at the time of illness, you may learn to avoid }%DIFDELCMD < {%%%
\DIFdel{all restaurants; A only; X only; all foods; specific pairing of X-in-A.}%DIFDELCMD < }%%%
...
cues that predict motivationally relevant outcomes (e.g.\DIFaddbegin \DIFadd{\ }\DIFaddend reward)
...
\cite{Day2007a,Roitman2005,Setlow2003,Schultz1997}\DIFdelbegin \DIFdel{(Day 2007 is the only paper referenced here that looks at transferring of responses from reward to CS+, other papers are in VTA, or the development of CS+ responses)}\DIFdelend .
...
much less is known about how \DIFdelbegin \DIFdel{reward-predictive }\DIFdelend \DIFaddbegin \DIFadd{outcome-predictive }\DIFaddend cues themselves are
...
activity needs to be maintained \cite{Lee2012,sutton1998}. \DIFaddbegin \DIFadd{For
...
maintained. }\DIFaddend Since NAc is a primary target of DA signals interpretable
as \DIFdelbegin \DIFdel{RPEs}\DIFdelend \DIFaddbegin \DIFadd{reward prediction errors (RPEs)}\DIFaddend , and NAc lesions impair RPEs
...
identity of a preceding cue\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Holland1992}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend , spatial or configural arrangements
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Honey2014}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Holland1992,Bouton1993,Honey2014}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , and unsignaled rules as
occurs in set shifting and other cognitive control tasks
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Grant1948}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Grant1948,cohen1992context,Floresco2006a,Sleezer2016}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . In such
situations, the question arises how selective, context-dependent
processing of \DIFdelbegin \DIFdel{reward-predictive }\DIFdelend \DIFaddbegin \DIFadd{outcome-predictive }\DIFaddend cues is implemented. For instance, is
...
. }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Goto2008,Fitzgerald2014,Sleezer2016}}\hspace{0pt}%DIFAUXCMD
. Representation of cue
...
activity of NAc \DIFdelbegin \DIFdel{neurons }\DIFdelend \DIFaddbegin \DIFadd{units }\DIFaddend as rats performed a task in which multiple,
...
\caption{Schematic of potential coding strategies for cue identity
  ...
  reward-unavailable) employed by single units in the NAc \DIFaddbeginFL \DIFaddFL{across
  different units (A) and in time (B)}\DIFaddendFL . \bsf{A}: Displayed are
  schematic PETHs illustrating putative responses to different cues
  under different hypotheses of how cue identity and \DIFdelbeginFL \DIFdelFL{value }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{outcome }\DIFaddendFL are
  ...
  example demonstrating a unit that responds to \DIFdelbeginFL \DIFdelFL{reward-predictive }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{outcome-predictive
  }\DIFaddendFL cues, but firing rate is also modulated by cue identity, firing most
  ...
  revealed, '\DIFdelbeginFL \DIFdelFL{Cue-OFF}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{OFF}\DIFaddendFL ' signifies when the cue turns off. H2 (middle
  panel): Coding of cue identity \DIFdelbeginFL \DIFdelFL{is
  maintained }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{persists }\DIFaddendFL during a nosepoke hold
  ...
  (H2B). H3 (right panel): Coding of cue identity \DIFdelbeginFL \DIFdelFL{is
  maintained during entire duration of cue, including }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{persists }\DIFaddendFL after \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL outcome is \DIFdelbeginFL \DIFdelFL{revealed}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{received when the rat gets feedback about his decision,
  by either the same unit as during cue-onset (H3A) or by a sequence of units (H3B)}\DIFaddendFL . The
...
then \DIFdelbegin \DIFdel{shaped to run on the task for sucrose
...
availability and inavailability of reward were then introduced }\DIFdelend \DIFaddbegin \DIFadd{trained on the behavioral task }\DIFaddend (described in \DIFdelbegin \DIFdel{detail below, and rats were trained for 200 trials per day until they discriminated between the cues}\DIFdelend \DIFaddbegin \DIFadd{the next section)
until they reached performance criterion}\DIFaddend . At this point they
...
pre-surgery levels. Upon completion of recording, animals were \DIFaddbegin \DIFadd{gliosed,
}\DIFaddend euthanized and recording sites were histologically confirmed.

{\bf Behavioral task and training:}

\DIFdelbegin \DIFdel{Rats were trained to run clockwise on }\DIFdelend \DIFaddbegin \DIFadd{The behavioral apparatus was }\DIFaddend an elevated, square-shaped track (100 x
100 cm, track width 10 cm) containing four possible reward locations
\DIFaddbegin \DIFadd{at the end of track ``arms'' }\DIFaddend (Figure \ref{fig:task}). Rats initiated a
{\it trial} by \DIFdelbegin \DIFdel{running down the length of an
arm, and }\DIFdelend triggering a photobeam located 24 cm from the start of
...
amplitude was modulated from 0 to maximum by a 2 Hz sine wave. \DIFdelbegin \DIFdel{Reward-cue
}\DIFdelend \DIFaddbegin \DIFadd{Outcome-cue
}\DIFaddend associations were counterbalanced across rats, e.g.\ for some rats L1+ was the
...
period on the pedestal. \DIFaddbegin \DIFadd{For approximately the first week of training, rats were restricted to
...
start of training. }\DIFaddend Rats were trained \DIFdelbegin \DIFdel{daily until they could distinguish
}\DIFdelend \DIFaddbegin \DIFadd{for 200 trials per day (100
trials per block) until they discriminated }\DIFaddend between the reward-available and reward-unavailable cues for both light and
...
reward-unavailable trials, at which point they underwent \DIFdelbegin \DIFdel{eletrode }\DIFdelend \DIFaddbegin \DIFadd{electrode }\DIFaddend implant
surgery.
...
the availability of \DIFaddbeginFL \DIFaddFL{12\% }\DIFaddendFL sucrose reward at the nearest reward receptacle (light
...
\DIFdelbeginFL \DIFdelFL{nosepokes}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Nosepokes (solid lines)}\DIFaddendFL . Rectangular boxes with yellow fill indicate location of LEDs used
for light cues. Speakers for tone cues were placed underneath the choice
points, indicated by magenta fill on track. \DIFdelbeginFL \DIFdelFL{Light blue fill on ends }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Arrows outside }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{arms
  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{track }\DIFaddendFL indicate \DIFdelbeginFL \DIFdelFL{location of receptacles where sucrose was dispensed on rewarded
  trials}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{correct running direction}\DIFaddendFL . Circle in the center indicates location of pedestal during pre- and post-records. Scale bar is located beneath the track. \bsf{B}: Progression of
a recording session. A session was started with a 5 minute recording period on
a pedestal placed in the center of the apparatus. Rats then performed \DIFdelbeginFL \DIFdelFL{two
  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the light and sound }\DIFaddendFL blocks of \DIFdelbeginFL \DIFdelFL{a }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL cue discrimination task \DIFdelbeginFL \DIFdelFL{on a square track where they had to
  dissociate between a reward-available and reward-unavailable cue }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{in succession }\DIFaddendFL for \DIFdelbeginFL \DIFdelFL{a set of
  light cues and a set of sound cues for a target of }\DIFdelendFL 100 trials each\DIFdelbeginFL \DIFdelFL{. Rats ran
  clockwise on the square track}\DIFdelendFL , \DIFdelbeginFL \DIFdelFL{and upon presentation of the cue could either
  ...
  presented in a row. Location of the cue }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{followed by another 5 minute recording period }\DIFaddendFL on the \DIFdelbeginFL \DIFdelFL{track was irrelevant for
  behavior, all cue locations contained an equal amount of reward-available and
  reward-unavailable trials}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{pedestal}\DIFaddendFL . Left in figure depicts a light block, showing an
example trajectory for a correct reward-available (\DIFaddbeginFL \DIFaddFL{approach trial; }\DIFaddendFL red) and reward-unavailable
(\DIFaddbeginFL \DIFaddFL{skip trial; }\DIFaddendFL green) trial. Right in figure depicts a sound block, with a reward-available
(\DIFaddbeginFL \DIFaddFL{approach trial; }\DIFaddendFL navy blue) and reward-unavailable (\DIFaddbeginFL \DIFaddFL{skip trial; }\DIFaddendFL light blue) trial. Ordering of the light and sound blocks was counterbalanced across sessions. \DIFdelbeginFL \DIFdelFL{Dashed line within track
  indicates photobeam }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Reward-available and reward-unavailable cues were presented pseudo-randomly, such }\DIFaddendFL that \DIFdelbeginFL \DIFdelFL{triggered }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{not more than two of the same type of }\DIFaddendFL cue \DIFdelbeginFL \DIFdelFL{onset. Solid line within track
  indicates photobeam that registered }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{could be presented in }\DIFaddendFL a \DIFdelbeginFL \DIFdelFL{nosepoke into the reward
  receptacles}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{row}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{Receptacles containing sucrose solution are present within }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Location of }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{squares at }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cue on }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{end }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{track was irrelevant for
behavior, all cue locations contained an equal amount }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{each arm. Light blue fill signifies reward receipt on a
  correct }\DIFdelendFL reward-available \DIFdelbeginFL \DIFdelFL{trial}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and
reward-unavailable trials}\DIFaddendFL .
\DIFdelbeginFL \DIFdelFL{Arrows outside of track indicate correct
  running direction. A session ended with another 5 minute recording period on
  the pedestal.}\DIFdelendFL }
\label{fig:task}
...
\DIFdelbegin \DIFdel{(Malhotra et al. ,
2015). }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Malhotra2015}}\hspace{0pt}%DIFAUXCMD
. }\DIFaddend Briefly, animals were administered analgesics and
...
bregma; \DIFdelbegin \DIFdel{Paxinos and Watson, 2005}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeNP{atlas}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend ), or 12 in the right NAc and 4 targeted at the
...
between 600-9000 Hz, thresholding and aligning the peaks \DIFdelbegin \DIFdel{(UltraMegaSort2k,
Hull et al. , 2011). }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite[UltraMegaSort2k, ][]{Hill2011}}\hspace{0pt}%DIFAUXCMD
. }\DIFaddend Spike waveforms were then
...
A.D.\ Redish et al.\DIFaddbegin \DIFadd{, http://redishlab.neuroscience.umn.edu/MClust/MClust.html}\DIFaddend ). Isolated units containing a minimum of 200
...
ISIs \DIFdelbegin \DIFdel{(Barnes 2005, Atallah 2014)}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
...
predictor for a given unit. \DIFaddbegin \DIFadd{To investigate more finely the temporal dynamics of the influence of task parameters to unit activity, we then fit a sliding window GLM with the
...
excluded 99 \DIFaddbegin \DIFadd{of 443 }\DIFaddend units from analysis that showed a significant change for
either block\DIFaddbegin \DIFadd{, leaving 344 units for further analyses}\DIFaddend . All analyses were completed in MATLAB R2015a, the code
...
Upon completion of the experiment, \DIFdelbegin \DIFdel{rats were }\DIFdelend \DIFaddbegin \DIFadd{recording channels were gliosed by passing 150 $\mu A$ current for 10 seconds and waiting 5 days before euthanasia, except for rat R057 whose implant detached prematurely. Rats were }\DIFaddend anesthetized with 5\%
...
\caption{Histological verification of recording sites. Upon completion of
  ...
  NAc core just dorsal to the anterior commissure (\DIFdelbeginFL \DIFdelFL{white circle}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{arrow}\DIFaddendFL ). \bsf{B}:
  Schematic showing recording areas for \DIFdelbeginFL \DIFdelFL{the four rats used}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{all subjects}\DIFaddendFL .}
\label{fig:histo}
\end{figure}
\section*{Results}
...
identical arms for two distinct \DIFdelbegin \DIFdel{sets }\DIFdelend \DIFaddbegin \DIFadd{set }\DIFaddend of cues (Figure
...
approach was influenced by \DIFdelbegin \DIFdel{which of the cues was presented }\DIFdelend \DIFaddbegin \DIFadd{whether a reward-available or reward-unavailable cue was presented, but was not significantly modulated by whether the rat was presented with a light or sound cue }\DIFaddend (Percentage
approached: light reward-available = 97\%; light reward-unavailable =
34\%; sound reward-available = 91\%; sound reward-unavailable 35\%; \DIFdelbegin \DIFdel{p }\DIFdelend \DIFaddbegin \DIFadd{cue identity p
$=$ .115; cue outcome p}\DIFaddend $<$ .001\DIFdelbegin \DIFdel{), but that cue identity did not
significantly affect }\DIFdelend \DIFaddbegin \DIFadd{; Figure \ref{fig:behav}B). A similar trend was seen with }\DIFaddend the
length of time taken to complete a trial (Trial length: light
reward-available = 1.85 s; light reward-unavailable = 1.74 s; sound
reward-available = 1.91 s; sound reward-unavailable 1.78 s; \DIFaddbegin \DIFadd{cue identity }\DIFaddend p $=$ \DIFdelbegin \DIFdel{.13; }\DIFdelend \DIFaddbegin \DIFadd{.115; cue outcome p $<$ .001; }\DIFaddend Figure \ref{fig:behav}C\DIFdelbegin \DIFdel{,D}\DIFdelend ). Thus, during recording, rats successfully
...
\caption{Performance on the behavioral task. \DIFdelbeginFL \DIFdelFL{\bsf{A-B}}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\bsf{A}}\DIFaddendFL . Example learning curves
...
line for sound block) \DIFaddbeginFL \DIFaddFL{for light (top) and sound (bottom) blocks}\DIFaddendFL . Fully correct performance corresponds to \DIFaddbeginFL \DIFaddFL{an }\DIFaddendFL approach
...
surgery. \DIFdelbeginFL \DIFdelFL{\bsf{C-D}}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\bsf{B-C}}\DIFaddendFL : Summary of performance during recording sessions for each
rat. \DIFdelbeginFL \DIFdelFL{\bsf{C}}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\bsf{B}}\DIFaddendFL : Proportion approached for all rats, averaged across all
...
for statistics). \DIFdelbeginFL \DIFdelFL{\bsf{D}}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\bsf{C}}\DIFaddendFL : Average trial length for each cue. Note that the
...
\DIFdelbegin \DIFdel{independently }\DIFdelend encodes features of reward-predictive cues separate from expected
...
cue (Figure \ref{fig:examples}G,H). \DIFdelbegin \DIFdel{Fitting a GLM to all recorded units revealed a similar pattern of results (data not shown}\DIFdelend \DIFaddbegin \DIFadd{Additionally, a sliding window GLM centered on cue-onset revealed that cue identity and cue location contributed to the activity
of a significant proportion of cue-modulated units throughout this epoch, whereas an increase in units encoding cue outcome became apparent after cue-onset (Figure \ref{fig:GLM}D}\DIFaddend ).  Together, these findings show that various cue features are represented in the NAc, 
and that this coding \DIFdelbegin \DIFdel{has
some overlap, but is mostly distinct }\DIFdelend \DIFaddbegin \DIFadd{is both integrated and separate }\DIFaddend from expected outcome (Figure
...
[h]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{Fig 6 - GLM.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=0.5\textheight]{Fig 6 - GLM.png}
...
of MSNs and FSIs. \DIFaddbeginFL \DIFaddFL{Scale bar indicates range of improvements to model fit for units with an increasing (green) and decreasing (red) response to the cue. }\DIFaddendFL \bsf{B}: Violin plots demonstrating changes in R-squared
...
\caption{Population-level averages of cue feature sensitive NAc units. \bsf{A}:
  ...
  nonpreferred stimulus block (blue). Dashed \DIFaddbeginFL \DIFaddFL{vertical }\DIFaddendFL line indicates onset of
  cue. \DIFaddbeginFL \DIFaddFL{Dashed color lines indicate the result of shuffling the identity of the units used for this average 1000 times. }\DIFaddendFL Lightly shaded area indicates standard error of the mean. Note larger
  increase to preferred stimulus block \DIFdelbeginFL \DIFdelFL{to }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{over }\DIFaddendFL nonpreferred stimulus block. Black
  ...
  activity reveals \DIFdelbeginFL \DIFdelFL{cells }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{units }\DIFaddendFL classified as “decreasing” in response to cue show a
  ...
  blocks. \bsf{C-D}: Same as A-B for cue location. Activity is plotted \DIFdelbeginFL \DIFdelFL{from }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for
  }\DIFaddendFL most preferred arm (\DIFdelbeginFL \DIFdelFL{yellow}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{black}\DIFaddendFL ) \DIFdelbeginFL \DIFdelFL{, in decreasing order to }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL least preferred arm (\DIFdelbeginFL \DIFdelFL{black,
  navy blue, magneta, respectively}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{magenta}\DIFaddendFL ). \DIFdelbeginFL \DIFdelFL{Note the graded response to arms of
  decreasing preference. }\DIFdelendFL \bsf{E-F}: Same as A-B for cue outcome. Activity is
  plotted for preferred expected outcome (red), and nonpreferred outcome
  (green). Note the larger increase to the cue representing the \DIFdelbeginFL \DIFdelFL{cell}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{unit}\DIFaddendFL ’s preferred
...
\DIFdelbegin \DIFdel{blocks. }%DIFDELCMD < {\color{red}%%%
\DIFdel{(JIMMIE EDIT THIS SECTION!)}%DIFDELCMD < } %%%
\DIFdelend \DIFaddbegin \DIFadd{Blocks. }\DIFaddend This revealed that while there was some preservation of order, the overall firing was qualitatively different
across the two blocks\DIFaddbegin \DIFadd{, implying that population activity distinguishes between light and sound blocks}\DIFaddend . To control for the possibility that any comparison of
...
trials vs half of sound trials) \DIFdelbegin \DIFdel{, }\DIFdelend suggesting that activity in the NAc
discriminates across various cue conditions (within block correlations = \DIFdelbegin \DIFdel{.381, }\DIFdelend \DIFaddbegin \DIFadd{.383 (light), .379 (sound); }\DIFaddend across block correlations = \DIFdelbegin \DIFdel{.342}\DIFdelend \DIFaddbegin \DIFadd{.343, .338, .337, .348; within block vs./ within block comparison = p $=$ .934; within block vs./ across block comparisons = p $<$ .001}\DIFaddend ). This process was repeated for cue location \DIFaddbegin \DIFadd{(Figure \ref{fig:tiling}C-D; within block correlations = .369 (arm 1), .350 (arm 2); across block correlations = .290, .286, .285, .291; within block vs./ within block comparison = p $=$ .071; within block vs./ across block comparisons = p $<$ .001) }\DIFaddend and cue outcome \DIFaddbegin \DIFadd{(Figure \ref{fig:tiling}E-F;  within block correlations = .429 (reward-available), .261 (reward-unavailable); across block correlations = .258}\DIFaddend , \DIFaddbegin \DIFadd{.253, .255, .249; within block vs./ within block comparison = p $<$ .001; within block vs./ across block comparisons = p $<$ .001), }\DIFaddend showing that NAc segmentation of the task is qualitatively
...
more similar to the across condition comparisons\DIFdelbegin \DIFdel{(Figure \ref{fig:tiling}C-F)}\DIFdelend \DIFaddbegin \DIFadd{, potentially due do the greater behavioral variability for the reward-unavailable trials}\DIFaddend .
...
firing rates. \DIFdelbeginFL \DIFdelFL{A}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\bsf{A}}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{far }\DIFdelendFL left: Heat plot showing smoothed normalized firing
...
captured by the peak firing rates of various units. A, middle\DIFdelbeginFL \DIFdelFL{left}\DIFdelendFL : Same units
...
a clear diagonal of elevated firing rates. A, \DIFdelbeginFL \DIFdelFL{middle }\DIFdelendFL right: Unit firing rates
...
indicating partial similarity between firing rates in the two blocks. A \DIFdelbeginFL \DIFdelFL{, far
  right: Unit firing rates taken from half of the light block, ordered according
  to peak firing rate taken from the other half of light trials. Note }\DIFdelendFL similar pattern \DIFdelbeginFL \DIFdelFL{to middle right suggests }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{exists for within-block comparisons suggesting }\DIFaddendFL that reordering any two sets of trials
...
representations of cue features \DIFdelbegin \DIFdel{were maintained }\DIFdelend \DIFaddbegin \DIFadd{persisted }\DIFaddend post-approach until the outcome
...
overlap greater than expected by chance according to chi-square \DIFdelbegin \DIFdel{tets}\DIFdelend \DIFaddbegin \DIFadd{tests}\DIFaddend ), and those
that did not previously have a cue feature as a predictor (29, 48, and 30 \DIFaddbegin \DIFadd{out of 133 cue-modulated }\DIFaddend units encoded both time points for cue identity, \DIFaddbegin \DIFadd{cue }\DIFaddend location, and \DIFaddbegin \DIFadd{cue }\DIFaddend outcome,
respectively). \DIFdelbegin \DIFdel{Fitting a GLM to all recorded units revealed a similar pattern
(data not shown). }\DIFdelend Population level averages for units that increased to
...
\DIFdelbegin \DIFdel{reward }\DIFdelend \DIFaddbegin \DIFadd{outcome }\DIFaddend receipt for all cue conditions where the rat would have received reward
(Figure \ref{fig:NP_tiling}). To determine whether coding of cue features \DIFdelbegin \DIFdel{was
maintained }\DIFdelend \DIFaddbegin \DIFadd{persisted }\DIFaddend after the outcome was revealed, a GLM was fit to the firing rates of
cue-modulated units at the time of \DIFdelbegin \DIFdel{reward }\DIFdelend \DIFaddbegin \DIFadd{outcome }\DIFaddend receipt, during which the cue was
...
shown). \DIFdelbegin \DIFdel{However, an }\DIFdelend \DIFaddbegin \DIFadd{An }\DIFaddend absence of cue identity or cue location coding \DIFaddbegin \DIFadd{at this level of analysis }\DIFaddend was observed,
\DIFaddbegin \DIFadd{but looking at the data more closely with a sliding window GLM revealed that
...
}\DIFaddend suggesting that the NAc \DIFdelbegin \DIFdel{does not maintain }\DIFdelend \DIFaddbegin \DIFadd{maintains }\DIFaddend a representation of these cue features once the rat receives behavioral feedback for its decision \DIFaddbegin \DIFadd{(Figure \ref{fig:NP_GLM}E-J)}\DIFaddend .
...
[h]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{Fig 10 - NP GLM.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=0.4\textheight]{Fig 10 - NP GLM.png}
\DIFaddendFL \caption{Summary of influence of various task parameters of cue-modulated NAc
units during nosepoke. \bsf{A}: GLM matrix \DIFdelbeginFL \DIFdelFL{demonstrating impact }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{illustrating the contribution }\DIFaddendFL of various
task parameters \DIFdelbeginFL \DIFdelFL{on }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{to }\DIFaddendFL NAc \DIFaddbeginFL \DIFaddFL{unit }\DIFaddendFL firing rates. A stepwise GLM was fit to each unit that
showed evidence of cue modulation by a Wilcoxon signed-rank test. Each row
represents a given task parameter, and each column \DIFdelbeginFL \DIFdelFL{is the influence of that
  task parameter on }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{corresponds to }\DIFaddendFL a \DIFdelbeginFL \DIFdelFL{given }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{single }\DIFaddendFL unit. \DIFdelbeginFL \DIFdelFL{Response variable is }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Colors indicate }\DIFaddendFL how much of the firing rate variance an
individual predictor contributed to the model, as measured by differences in
R-squared between the final model and the model minus the predictor of
\DIFdelbeginFL \DIFdelFL{interest}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Interest}\DIFaddendFL . Ordering from left to right: MSNs that increased firing
...
\caption{Population-level averages of cue feature sensitive NAc units during a
  nosepoke. \bsf{A}: Average smoothed normalized \DIFaddbeginFL \DIFaddFL{(z-score) }\DIFaddendFL activity for cue-modulated
  ...
  (blue). Black \DIFaddbeginFL \DIFaddFL{vertical }\DIFaddendFL dashed line indicates nosepoke. Red \DIFaddbeginFL \DIFaddFL{vertical }\DIFaddendFL dashed line indicates reward
  delivery occurring 1 s after nosepoke for reward-available trials. \DIFaddbeginFL \DIFaddFL{Dashed color lines indicate the result of shuffling the identity of the units used for this average 1000 times. }\DIFaddendFL Lightly
  shaded area indicates standard error of the mean. Note larger increase leading
  up to nosepoke to preferred stimulus block \DIFdelbeginFL \DIFdelFL{to }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{over }\DIFaddendFL nonpreferred stimulus
  ...
  for cue location. Activity is plotted \DIFdelbeginFL \DIFdelFL{from }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for }\DIFaddendFL most preferred arm (\DIFdelbeginFL \DIFdelFL{yellow}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{black}\DIFaddendFL ) \DIFdelbeginFL \DIFdelFL{, in
  decreasing order to }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL least preferred arm (\DIFdelbeginFL \DIFdelFL{black, navy blue, }\DIFdelendFL magneta\DIFdelbeginFL \DIFdelFL{,
  respectively}\DIFdelendFL ). \DIFdelbeginFL \DIFdelFL{Note the subtle but graded response to arms of decreasing
  preference. }\DIFdelendFL \bsf{E-F}: Same as A-B for cue outcome. Activity is plotted for
  preferred expected outcome (red), and nonpreferred outcome (green). Note the
  peak after \DIFdelbeginFL \DIFdelFL{reward }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{outcome }\DIFaddendFL receipt for preferred outcome in decreasing \DIFdelbeginFL \DIFdelFL{cells }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{units }\DIFaddendFL (F).}
\label{fig:NP_pop}
\end{figure}
\begin{figure}
[h]
...
\caption{Distribution of NAc firing rates across \DIFdelbeginFL \DIFdelFL{task space during }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{time surrounding nosepoke for }\DIFaddendFL approach
trials. Each panel shows normalized \DIFaddbeginFL \DIFaddFL{(z-score) }\DIFaddendFL firing rates for all recorded NAc units
...
indicated by text labels. \DIFaddbeginFL \DIFaddFL{\bsf{A-C}: Heat plots aligned to normalized peak firing rates. }\DIFaddendFL \bsf{A}, far left: Heat plot showing smoothed
...
firing rates of various units. A, middle\DIFdelbeginFL \DIFdelFL{left}\DIFdelendFL : Same units ordered according to
...
elevated firing rates, and a clustering around \DIFdelbeginFL \DIFdelFL{reward }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{outcome }\DIFaddendFL receipt. A, \DIFdelbeginFL \DIFdelFL{middle
  }\DIFdelendFL right: Unit firing rates taken from the sound block, ordered according to peak
...
rates in the two blocks. A \DIFdelbeginFL \DIFdelFL{, far right: Unit firing rates taken from half of
  the light block, ordered according to peak firing rate taken from the other
  half of light trials. Note }\DIFdelendFL similar pattern \DIFdelbeginFL \DIFdelFL{to middle right suggests }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{exists for within-block comparisons suggesting }\DIFaddendFL that
...
(see text). \bsf{B}: Same layout as in A, \DIFdelbeginFL \DIFdelFL{but with units ordered according to
  the time of their minimum firing rate. \bsf{C}: Same layout as in A, }\DIFdelendFL except
...
example locations were used for display purposes; other location pairs showed
a similar pattern. \DIFdelbeginFL \DIFdelFL{\bsf{D}}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\bsf{C}}\DIFaddendFL : \DIFdelbeginFL \DIFdelFL{As for C, but with units ordered by their minimum
  firing rate. \bsf{E}: }\DIFdelendFL Same layout as in A, except that panels now compare
\DIFaddbeginFL \DIFaddFL{correct }\DIFaddendFL reward-available and \DIFaddbeginFL \DIFaddFL{incorrect }\DIFaddendFL reward-unavailable trials. \DIFdelbeginFL \DIFdelFL{Notice the absence of a
  cluster of }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The disproportionate }\DIFaddendFL tiling around \DIFdelbeginFL \DIFdelFL{reward }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{outcome }\DIFaddendFL receipt for \DIFaddbeginFL \DIFaddFL{reward-available, but not }\DIFaddendFL reward-unavailable
trials \DIFaddbeginFL \DIFaddFL{suggests encoding of reward receipt by NAc units}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{\bsf{F}}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\bsf{D-F}}\DIFaddendFL : \DIFdelbeginFL \DIFdelFL{As for }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Heat plots aligned to normalized minimum firing rates. \bsf{D}: Responses during different stimulus blocks as in }\DIFaddendFL A, but with units ordered \DIFaddbeginFL \DIFaddFL{according to
the time of their minimum firing rate. \bsf{E}: Responses during trials on different arms as in B, but with units ordered }\DIFaddendFL by their minimum
firing rate. \DIFaddbeginFL \DIFaddFL{\bsf{F}: Responses during cues signalling different outcomes as in C, but with units ordered by their minimum firing
rate. }\DIFaddendFL Overall, NAc units "tiled" experience on the task, as opposed to being
...
The \DIFdelbegin \DIFdel{present study found evidence for coding, by partially overlapping populations of NAc units , of multiple identifying features of motivationally relevant stimuli in addition to expected outcome ; the sensory modality of the presented cue, as well at its physical location on the
track (H2 and H3 in Figure \ref{fig:schematic}A)}\DIFdelend \DIFaddbegin \DIFadd{main result of the present study is that NAc units encode not
only the expected outcome of outcome-predictive cues, but also the
identity of such cues}\DIFaddend . Importantly, this identity coding was
maintained on approach trials during a delay period where the rat held
a nosepoke until \DIFdelbegin \DIFdel{receipt of outcome }\DIFdelend \DIFaddbegin \DIFadd{the outcome was received }\DIFaddend (H2 in Figure
\ref{fig:schematic}B). \DIFdelbegin \DIFdel{Cells }\DIFdelend \DIFaddbegin \DIFadd{Units coding for cue identity showed partial
overlap with those coding for expected outcome (H3 in Figure
\ref{fig:schematic}A). Units }\DIFaddend that coded different cue features
\DIFaddbegin \DIFadd{(identity, outcome, location) }\DIFaddend exhibited different temporal profiles as
a whole, although across all recorded \DIFdelbegin \DIFdel{cells }\DIFdelend \DIFaddbegin \DIFadd{units }\DIFaddend a tiling of task structure
...
\DIFdelbegin \DIFdel{cells}\DIFdelend \DIFaddbegin \DIFadd{units}\DIFaddend . Furthermore, this tiling differed between various conditions
with a cue feature, such as light versus sound blocks. \DIFaddbegin \DIFadd{We discuss
these observations and their implications below.
}\DIFaddend 

{\bf Cue identity:}

Our finding that NAc units can discriminate between \DIFaddbegin \DIFadd{different
}\DIFaddend outcome-predictive stimuli with similar \DIFdelbegin \DIFdel{biological significance
, in the current study cues from different sensory modalities, }\DIFdelend \DIFaddbegin \DIFadd{motivational significance
(i.e.\ encodes cue identity) }\DIFaddend expands upon an extensive rodent
literature examining NAc correlates of conditioned stimuli
\cite{Ambroggi2008,Atallah2014,Bissonette2013,Cooch2015,Day2006,Dejean2017,Goldstein2012,Ishikawa2008,Lansink2012,McGinty2013,Nicola2004,Roesch2009a,Roitman2005,Saddoris2011,Setlow2003,Sugam2014,West2016,Yun2004}. \DIFdelbegin \DIFdel{Units that discriminated across blocks were not simply due to drifting of the signal across trials, as units that showed a drift in firing between the first and second half within a block were excluded from the analysis. Furthermore, even though actions were stereotyped during correct trials, such that the rat always turned left at the decision point to approach for reward, and right to skip the receptacle and initiate the next trial, units that were modulated by the expected value of the cue maintained their specific firing patterns even during error trials where the rat turned left after presentation of the reward-unavailable cue, suggesting that these signals did not represent action values. Additionally, NAc signals have been shown to be modulated by response vigor, to detangle this from our results we included the trial length as a predictor in our GLMs, and found units with cue feature correlates independent of trial length \mbox{%DIFAUXCMD
...
\DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{such }{\it \DIFadd{outcome identity}} \DIFadd{coding }\DIFaddend as it shows that NAc units \DIFdelbegin \DIFdel{have representations of identifiable aspects of the cue
  itself}\DIFdelend \DIFaddbegin {\it \DIFadd{cue
  identity}}\DIFaddend , both separately and integrated with the reward it
predicts (H2 and H3 in Figure \ref{fig:schematic}A). \DIFdelbegin \DIFdel{Another study paired separate }\DIFdelend \DIFaddbegin \DIFadd{Similarly,
\mbox{%DIFAUXCMD
\citeNP{Setlow2003} }\hspace{0pt}%DIFAUXCMD
paired distinct }\DIFaddend cues with appetitive or aversive
outcomes, and found separate populations of units that encoded each
cue\DIFdelbegin \DIFdel{, with many switching selectivity after reversal of the associations between the cues and outcomes, providing evidence that the NAc encodes the biological significance of stimuli \mbox{%DIFAUXCMD
\cite{Setlow2003}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend . Once again, our study was different \DIFdelbegin \DIFdel{as we recorded neural responses to }\DIFdelend \DIFaddbegin \DIFadd{in asking how }\DIFaddend distinct cues
encoding the same anticipated outcome \DIFdelbegin \DIFdel{, suggesting }\DIFdelend \DIFaddbegin \DIFadd{are encoded. Such cue identity
encoding suggests }\DIFaddend that even when the biological relevance of these
stimuli is similar\DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend NAc dissociates their representations at the level
of the \DIFdelbegin \DIFdel{single-unit, although this doesn’t explain the difference in population averaged firing rate for the epochs when the cue is not present}\DIFdelend \DIFaddbegin \DIFadd{single-units}\DIFaddend . A possible interpretation of this coding of cue
...
\DIFdelbegin \DIFdel{Another possibility is that these modality specific units were encoding the }\DIFdelend \DIFaddbegin \DIFadd{A different possible function for cue identity coding is to support
...
not cue B is rewarded, whereas in }\DIFaddend context \DIFdelbegin \DIFdel{, rule, or sequence within a session as some units responded similarly for both reward-available and reward-unavailable cues within a block. This interpretation is in alignment with a recent paper from the non-human primate literature that }\DIFdelend \DIFaddbegin \DIFadd{2 cue B but not cue A is
rewarded. Successfully implementing such contextual mappings requires
representation of the cue identities. Indeed, \mbox{%DIFAUXCMD
\cite{Sleezer2016}
...
rule category was currently active\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Sleezer2016}}\hspace{0pt}%DIFAUXCMD
. Indeed, an encoding of the current strategy could be an explanation as to why a sustained difference in population averaged firing was seen across stimulus blocks, as well as a potential explanation for the differentially tiling of task structure across blocks in the current study. }\DIFdelend \DIFaddbegin \DIFadd{. }\DIFaddend Further support for a
...
overlapping, populations of units encoding cue \DIFdelbegin \DIFdel{modality and expected
value}\DIFdelend \DIFaddbegin \DIFadd{identity and expected
outcome}\DIFaddend , suggests that the fMRI finding is generated by the combined
activity of several different functional cell types.

\DIFdelbegin \DIFdel{A caveat of the }\DIFdelend \DIFaddbegin \DIFadd{Our analyses were designed to eliminate several potential alternative
...
\DIFadd{A different potential confound is that between outcome and action
...
been shown to be modulated by response vigor \mbox{%DIFAUXCMD
\cite{McGinty2013}}\hspace{0pt}%DIFAUXCMD
; to
...
\DIFadd{An overall limitation of the }\DIFaddend current study is that rats were never
presented with both sets of cues simultaneously, and \DIFdelbegin \DIFdel{thus never had }\DIFdelend \DIFaddbegin \DIFadd{were not required
}\DIFaddend to switch strategies \DIFdelbegin \DIFdel{, although extrapolating the data from the primate study suggests that the activity of the cue identity units }\DIFdelend \DIFaddbegin \DIFadd{between multiple sets of cues. Thus, it is
...
\mbox{%DIFAUXCMD
\cite{Sleezer2016} }\hspace{0pt}%DIFAUXCMD
suggests that cue identity coding }\DIFaddend would be
modulated by relevance. \DIFdelbegin \DIFdel{Keeping along this theme, the current data set is unable to identity precisely what the modality-sensitive neurons were encoding, that is were they tracking representations of stimulus identity, a preferred context, or even a macroscale representation of progress through the session. Furthermore, their relevance for ongoing behavior is also uncertain. }\DIFdelend NAc core lesions have been shown to impair
shifting between different behavioral strategies \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Floresco2006a}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend ,
...
\DIFdelbegin \DIFdel{strong }\DIFdelend inputs from the hippocampus, and the communication of place-reward
information across the two structures suggests that the NAc \DIFdelbegin \DIFdel{might be keeping track of }\DIFdelend \DIFaddbegin \DIFadd{tracks
}\DIFaddend locations associated with reward
\cite{Lansink2008,Lansink2009,Lansink2016,Pennartz2004,Sjulson2017,Tabuchi2000,VanderMeer2011}. \DIFdelbegin \DIFdel{Alternatively, it is possible that this positional modulation is part of a representation of task space, keeping track of where in the task the rat is. A previous non-human primate experiment showed that when reward is contingent upon completion of a series of trials, separate populations of NAc
neurons signal the start of a schedule, subsequent trials in the schedule, and the first trial in extended schedules \mbox{%DIFAUXCMD
\cite{Shidara1998}}\hspace{0pt}%DIFAUXCMD
. This signalling of position within a sequence has been observed in subsequent studies, and it is possible that the rats were keeping track of which specific arm they were in as part of a sequence of arms, especially since they started from the same arm during the start of each session \mbox{%DIFAUXCMD
\cite{Atallah2014,Berke2009,Khamassi2008,Lansink2012,Mulder2004}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{NAc
...
the vast majority of \DIFdelbegin \DIFdel{cells. A limitation of the current study is that the behavioral relevance of
cue location was not manipulated in the sense that varying cue location did not change the outcome-predictive nature of the cue}\DIFdelend \DIFaddbegin \DIFadd{units. In any case, NAc units on the present
...
events with the locations where they occur}\DIFaddend .

{\bf \DIFdelbegin \DIFdel{Potential functional consequences of maintained coding}\DIFdelend \DIFaddbegin \DIFadd{Implications}\DIFaddend :}

\DIFdelbegin \DIFdel{In the current study we found that the coding of cue features persisted after the choice point, during a delay period while the rat waited at the receptacle for the outcome (H2 in Figure \ref{fig:schematic}B). Having an enriched representation that includes details about the environmental context the animal finds itself in alongside expected outcome, and is maintained online until the outcome is revealed could be useful for assigning credit of a reward to the appropriate elements of an environment, to develop an accurate value function and optimize long term acquisition of reward \mbox{%DIFAUXCMD
...
. Interestingly, we found coding of cue identity and location after presentation }\DIFdelend \DIFaddbegin \DIFadd{Maladaptive decision making, as occurs in schizophrenia, addiction,
...
. However, the effects of RPE and value
updating are contingent upon encoding of preceding action and cue
features, the eligibility trace \mbox{%DIFAUXCMD
...
locations. While in our current study, the location and identity }\DIFaddend of
the cue \DIFdelbegin \DIFdel{, and during a delay period between nosepoke and outcome, but not after the outcome was received. The absence of coding during feedback, and that these recordings were done after behavioral performance had stabilized suggests the possibility that these NAc representations represent an integrated value estimate that is }\DIFdelend \DIFaddbegin \DIFadd{did not require any adjustments in the animal’s behavior, we
found coding of these features alongside the expected outcome of the
cue that could be }\DIFaddend the outcome of \DIFdelbegin \DIFdel{earlier upstream credit assignment in cortical inputs. From a motivated behavior standpoint, the presence of these enriched representations can help inform action selection involving Pavlovian behaviors such as conditioned approach \mbox{%DIFAUXCMD
\cite{DiCiano2001,Parkinson2000,Saunders2012}}\hspace{0pt}%DIFAUXCMD
, and after a
decision has been made the maintained representation can help to hold the
response in the face of competing alternatives while waiting for an outcome \mbox{%DIFAUXCMD
\cite{DiCiano2008,Floresco2008,Floresco2015,Peters2008}}\hspace{0pt}%DIFAUXCMD
.}\DIFdelend \DIFaddbegin \DIFadd{credit assignment computations
...
Parkinson’s \mbox{%DIFAUXCMD
\cite{Corlett2010,Kapur2003}}\hspace{0pt}%DIFAUXCMD
. Thus, our results provide a
...
\DIFaddbegin \DIFadd{OUTTAKES
}

\DIFaddend {\bf Tiling of task structure:}

\DIFdelbegin \DIFdel{Additionally, we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend found that the population of recorded units had a relatively
...
predictable trial series, \DIFdelbegin \DIFdel{neurons }\DIFdelend \DIFaddbegin \DIFadd{units }\DIFaddend represented state value of cue, and
...
{\bf \DIFdelbegin \DIFdel{Implications}\DIFdelend \DIFaddbegin \DIFadd{Potential functional consequences of persistent coding}\DIFaddend :}

\DIFdelbegin \DIFdel{Maladaptive decision making, as occurs in schizophrenia, addiction, Parkinson’s, among others, can result from dysfunctional RPE and value signals \mbox{%DIFAUXCMD
\cite{Frank2004,Gradin2011,Maia2011}}\hspace{0pt}%DIFAUXCMD
. This view has been successful in
explaining both positive and negative symptoms in schizophrenia, and deficits in learning from feedback in Parkinson’s \mbox{%DIFAUXCMD
\cite{Frank2004,Gradin2011}}\hspace{0pt}%DIFAUXCMD
. However, the effects of RPE and value
updating are contingent upon encoding of preceding action and cue features, the eligibility trace \mbox{%DIFAUXCMD
\cite{Lee2012,sutton1998}}\hspace{0pt}%DIFAUXCMD
. Value updates can only be performed on these aspects of preceding experience that are encoded when the update occurs. Therefore, maladaptive learning and decision making can result from not only aberrant RPEs but also from altered cue feature encoding. For instance, on this task the environmental stimulus that signaled the availability of
reward was conveyed by two distinct cues that were presented in four locations. While in our current study, the location and identity }\DIFdelend \DIFaddbegin \DIFadd{In the current study we found that the coding of cue features
...
identity and location after presentation }\DIFaddend of the cue\DIFdelbegin \DIFdel{did not require any adjustments in the animal’s behavior, we found coding of these features alongside the expected outcome of the cue that could be }\DIFdelend \DIFaddbegin \DIFadd{, and during a
...
integrated value estimate that is }\DIFaddend the outcome of \DIFdelbegin \DIFdel{credit assignment computations computed upstream
...
\cite{Kaczkurkin2013,Kaczkurkin2017,Lissek2014}}\hspace{0pt}%DIFAUXCMD
, and delusions observed in disorders such as
schizophrenia,
Alzheimer’s and Parkinson’s \mbox{%DIFAUXCMD
\cite{Corlett2010,Kapur2003}}\hspace{0pt}%DIFAUXCMD
. Thus, our results provide a neural window into the process of credit assignment, such that the extent and specific manner in which this process fails in e.
g. syndromes such as schizophrenia, obsessive-compulsive disorder, etc. can be experimentally accessed.
}\DIFdelend \DIFaddbegin \DIFadd{earlier upstream
...
\DIFdelbegin \DIFdel{Orphan paragraph? (also removed mixed selectivity paragraphs)
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend If these representations function as an eligibility trace, then there
...
reward procurement\DIFaddbegin \DIFadd{.
}

\DIFadd{A fundamental problem faced by all reinforcement learning agents is
...
not only a RPE but also a trace of the preceding actions and/or cues}\DIFaddend .
