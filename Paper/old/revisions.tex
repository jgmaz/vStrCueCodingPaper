...
%DIF ADD vStrCueCoding_final.tex   Tue Apr  3 10:34:59 2018
...
emphasizing a role in learning from \DIFdelbegin \DIFdel{RPEs
\mbox{%DIFAUXCMD
...
}\DIFdelend \DIFaddbegin \DIFadd{reward prediction errors (RPEs) (\mbox{%DIFAUXCMD
...
as \DIFdelbegin \DIFdel{reward prediction errors (RPEs)}\DIFdelend \DIFaddbegin \DIFadd{RPEs}\DIFaddend , and NAc lesions impair RPEs
...
%DIFDELCMD < {\bf %%%
\DIFdel{Overall timeline:}%DIFDELCMD < }
%DIFDELCMD < 
...
%DIFDELCMD < {\bf %%%
\DIFdel{Behavioral task and training:}%DIFDELCMD < }
%DIFDELCMD < 
...
signaled the presence (}%DIFDELCMD < {\it %%%
\DIFdel{reward-available trial}%DIFDELCMD < }%%%
\DIFdel{, L1+, S1+) or
absence (}%DIFDELCMD < {\it %%%
\DIFdel{reward-unavailable trial}%DIFDELCMD < }%%%
\DIFdel{, L2-, S2-) of a 12\% sucrose
water reward (0.1 mL) at the upcoming reward site. A trial was
classified as an }%DIFDELCMD < {\it %%%
\DIFdel{approach trial}%DIFDELCMD < } %%%
\DIFdel{if the rat turned left at the
decision point and made a nosepoke at the reward receptacle (40 cm
from the decision point), while trials were classified as a }%DIFDELCMD < {\it %%%
\DIFdel{skip
trial}%DIFDELCMD < } %%%
...
second delay between a nosepoke and subsequent reward delivery. }%DIFDELCMD < {\it
%DIFDELCMD < %%%
\DIFdel{Trial length}%DIFDELCMD < } %%%
...
%DIFDELCMD < %%%
\DIFdel{Each session consisted of both a }%DIFDELCMD < {\it %%%
\DIFdel{light block}%DIFDELCMD < } %%%
\DIFdel{and a }%DIFDELCMD < {\it %%%
\DIFdel{sound block}%DIFDELCMD < } %%%
\DIFdel{with
...
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=\textwidth]{Fig 2 - Schematic task.png}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
%DIFAUXCMD
%DIFDELCMD < \label{fig:task}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < {\bf %%%
\DIFdel{Surgery:}%DIFDELCMD < }
%DIFDELCMD < 
...
%DIFDELCMD < {\bf %%%
\DIFdel{Data acquisition and preprocessing:}%DIFDELCMD < }
%DIFDELCMD < 
...
between 600-9000 Hz, thresholding and aligning the peaks \mbox{%DIFAUXCMD
...
%DIFDELCMD < {\it %%%
\DIFdel{Behavior.}%DIFDELCMD < } %%%
\DIFdel{To determine if rats distinguished behaviorally
between the reward-available and reward-unavailable cues (}%DIFDELCMD < {\it %%%
\DIFdel{cue
outcome}%DIFDELCMD < }%%%
...
\DIFdel{cue outcome}%DIFDELCMD < } %%%
\DIFdel{(reward available or not) and }%DIFDELCMD < {\it %%%
...
the 1 s following cue-onset. Single units were considered to be }%DIFDELCMD < {\it
%DIFDELCMD < %%%
\DIFdel{cue-modulated}%DIFDELCMD < } %%%
...
%DIFDELCMD < \begin{figure}[h]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=\textwidth]{Fig 3 - Histology.png}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
\begin{figure}[h]
\centering
\DIFaddbeginFL \includegraphics[width=\textwidth]{Fig 2 - Schematic task.png}
\caption{\DIFaddFL{Schematic of behavioral task. \bsf{A}: To scale depiction of square
track consisting of multiple identical T-choice points. At each choice point,
the availability of 12\% sucrose reward at the nearest reward receptacle (light
blue fill) was signaled by one of four possible cues, presented when the rat
initiated a trial by crossing a photobeam on the track (dashed
...
\DIFdelbegin \DIFdel{OUTTAKES
}\DIFdelend \DIFaddbegin \section*{\DIFadd{Methods}}
\DIFaddend 

{\bf \DIFdelbegin \DIFdel{Tiling of task structure}\DIFdelend \DIFaddbegin \DIFadd{Subjects:}}

\DIFadd{Adult male Long-Evans rats (n = 4, Charles River, Saint Constant, QC)
were used as subjects. Rats were individually housed with a 12/12-h
light-dark cycle, and tested during the light cycle. Rats were food
deprived to 85-90\% of their free feeding weight (weight at time of
implantation was 440 - 470 g), and water restricted 4-6 hours before
...
{\bf \DIFadd{Overall timeline}\DIFaddend :}

\DIFdelbegin \DIFdel{We found that the population of recorded units had a relatively
uniform distribution of firing fields within our taskspace, similar
to
...
in the light or sound block, suggesting that
...
. Likewise, the tiling we saw could be a consequence of upstream cortical or limbic
...
throughout a
task within a }\DIFdelend \DIFaddbegin \DIFadd{Each rat was first handled for seven days during which they were
exposed to the experiment room, the sucrose solution used as a
reinforcer, and the click of the sucrose dispenser valves. Rats were
then trained on the behavioral task (described in the next section)
until they reached performance criterion. At this point they
...
{\bf \DIFadd{Behavioral task and training:}}

\DIFadd{The behavioral apparatus was an elevated, square-shaped track (100 x
100 cm, track width 10 cm) containing four possible reward locations
at the end of track ``arms'' (Figure \ref{fig:task}). Rats initiated a
}{\it \DIFaddend trial\DIFaddbegin } \DIFadd{by triggering a photobeam located 24 cm from the start of
each arm. Upon trial initiation, one of two possible light cues (L1}\DIFaddend ,
\DIFdelbegin \DIFdel{but also higher-order task
information, like blocks. Furthermore, dopamine levels in the
NAc
fluctuate through a trial, and it is possible that the observed tiling
could be }\DIFdelend \DIFaddbegin \DIFadd{L2), or one of two possible sound cues (S1, S2), was presented that
signaled the presence (}{\it \DIFadd{reward-available trial}}\DIFadd{, L1+, S1+) or
absence (}{\it \DIFadd{reward-unavailable trial}}\DIFadd{, L2-, S2-) of a 12\% sucrose
water reward (0.1 mL) at the upcoming reward site. A trial was
classified as an }{\it \DIFadd{approach trial}} \DIFadd{if the rat turned left at the
decision point and made a nosepoke at the reward receptacle (40 cm
from the decision point), while trials were classified as a }{\it \DIFadd{skip
trial}} \DIFadd{if the rat instead turned right at the decision point and
triggered the photobeam to initiate the next trial. A trial is labeled
}{\it \DIFadd{correct}} \DIFadd{if the rat approached (i.e.\ nosepoked) on
reward-available trials, and skipped (i.e.\ did not nosepoke) on
reward-unavailable trials. On reward-available trials there was a 1
second delay between a nosepoke and subsequent reward delivery. }{\it
\DIFadd{Trial length}} \DIFadd{was determined by measuring the length of time from
cue onset until nosepoke (for approach trials), or from cue onset
until the start of the following trial (for skip trials). Trials could
only be initiated through clockwise progression through the series of
arms, and each entry into the subsequent arm on the track counted as a
...
\DIFadd{Each session consisted of both a }{\it \DIFadd{light block}} \DIFadd{and a }{\it \DIFadd{sound block}} \DIFadd{with
100 trials each. Within a block, one cue signaled reward was available on that
trial (L1+ or S1+), while the other signaled reward was not available (L2- or
S2-). Light block cues were a flashing white light, and a constant yellow
light. Sound block cues were a 2 kHz sine wave and a 8 kHz sine wave whose
...
presented more than twice in a row. Block order within each day was also
pseudorandomized, such that the rat could not begin a session with the same
block for more than two days in a row. Each session consisted of }\DIFaddend a \DIFdelbegin \DIFdel{NAc-representation of state value related to
this
temporally evolving dopamine signal. Future experiments should monitor
this mapping of task structure during the
application of dopamine
antagonists. Finally, the presence of functional correlates not
evident when looking at single-unit responses time-locked to
salient
task events emphasizes the need to employ ensemble level analyses
across all aspects of a }\DIFdelend \DIFaddbegin \DIFadd{5 minute
pre-session period on a pedestal (a terracotta planter filled with towels),
followed by the first block, then the second block, then a 5 minute post-session
period on the pedestal. For approximately the first week of training, rats were restricted to
running in the clockwise direction by presenting a physical barrier to
...
\DIFadd{Surgical procedures were as described previously
\mbox{%DIFAUXCMD
\cite{Malhotra2015}}\hspace{0pt}%DIFAUXCMD
. Briefly, animals were administered analgesics and
antibiotics, anesthetized with isoflurane, induced with 5\% in medical
...
at least five days to recover while receiving post-operative care, and
tetrodes were lowered to the target (DV -6.0 mm) before being
reintroduced to the behavioral }\DIFaddend task.

{\bf \DIFdelbegin \DIFdel{Potential functional consequences of persistent coding}\DIFdelend \DIFaddbegin \DIFadd{Data acquisition and preprocessing}\DIFaddend :}

\DIFdelbegin \DIFdel{In the current study we found that the coding of cue features
...
alongside expected outcome, }\DIFdelend \DIFaddbegin \DIFadd{After recovery, rats were placed back on the task for recording. NAc
signals were acquired at 20 kHz with a RHA2132 v0810 preamplifier
(Intan) and a KJE-1001/KJD-1000 data acquisition system
(Amplipex). Signals were referenced against a tetrode placed in the
corpus callosum above the NAc. Candidate spikes for sorting into
...
{\bf \DIFadd{Data analysis:}}

{\it \DIFadd{Behavior.}} \DIFadd{To determine if rats distinguished behaviorally
between the reward-available }\DIFaddend and \DIFdelbegin \DIFdel{is maintained online until the outcome
is revealed could be useful for assigning credit of a reward to }\DIFdelend \DIFaddbegin \DIFadd{reward-unavailable cues (}{\it \DIFadd{cue
outcome}}\DIFadd{), we generated linear mixed effects models to investigate
the relationships between cue type and our behavioral variables, with
}{\it \DIFadd{cue outcome}} \DIFadd{(reward available or not) and }{\it \DIFadd{cue identity}}
\DIFadd{(light or sound) as fixed effects, and }\DIFaddend the \DIFdelbegin \DIFdel{appropriate elements of an environment, to develop an accurate value
...
for simultaneous coding of outcome and outcome-predictive stimuli at the time of feedback in the dorsolateral PFC,
and that this coding of stimulus information was strongest before behavioral performance
stabilized \mbox{%DIFAUXCMD
\cite{Asaad2017}}\hspace{0pt}%DIFAUXCMD
. Interestingly, we found coding of cue
identity and location after presentation of the cue , }\DIFdelend \DIFaddbegin \DIFadd{addition of an intercept
for rat identity as a random effect. For each cue, the average
proportion of trials approached and trial length for a session were
used as response variables. Contribution of cue outcome to behavior
was determined by comparing the full model to a model with cue outcome
...
{\it \DIFadd{Neural data.}} \DIFadd{To investigate the contribution of different cue
features }{\DIFadd{cue identity and cue outcome) on the firing rates of NAc
single units, we first determined whether firing rates for a unit
were modulated by the onset of a cue by collapsing across all cues
and comparing the firing rates for the 1 s preceding cue-onset with
the 1 s following cue-onset. Single units were considered to be }{\it
\DIFadd{cue-modulated}} \DIFadd{if a Wilcoxon signed-rank test comparing pre- }\DIFaddend and
\DIFdelbegin \DIFdel{during a
delay period between nosepoke and outcome, but not after the
outcome
...
standpoint, }\DIFdelend \DIFaddbegin \DIFadd{post-cue firing was significant at $p <$ .01. Cue-modulated units
were then classified as either increasing or decreasing if the
post-cue activity was higher or lower than the pre-cue activity,
respectively.
}

\DIFadd{To determine the relative contribution of different task parameters to
firing rate variance (as in Figures \ref{fig:examples}-\ref{fig:GLM}),
a forward selection stepwise general linear model (GLM) was fit to
each cue-modulated unit. Cue identity (light block, sound block), cue
location (arm 1, arm 2, arm 3, arm 4), cue outcome (reward-available,
...
given task parameter if addition of the parameter significantly
improved model fit using deviance as the criterion ($p <$ .01). A
comparison of the R-squared value between the final model and }\DIFaddend the
\DIFdelbegin \DIFdel{presence of these enriched representations can help
inform action selection involving Pavlovian behaviors such as conditioned approach \mbox{%DIFAUXCMD
...
. }\DIFdelend \DIFaddbegin \DIFadd{final model minus the predictor of interest was used to determine the
amount of firing rate variance explained by the addition of that
predictor for a given unit. To investigate more finely the temporal dynamics of the influence of task parameters to unit activity, we then fit a sliding window GLM with the
same task parameters using 500 ms bins and 100 ms steps, starting 500 ms before cue-onset, up to 500 ms after cue-onset, and measured the proportion of units and average R-squared value for a given time bin
...
\DIFdelbegin \DIFdel{If these representations function as an eligibility trace, then there
...
associations between reinforcers and their environment,
which could
contribute to the presence of
prediction errors in the NAc in those
...
}\DIFdelend \DIFaddbegin \DIFadd{To better visualize responses to cues and enable subsequent population
level analyses (as in Figures \ref{fig:examples}, \ref{fig:pop}, and
\ref{fig:tiling}), spike trains were convolved with a Gaussian kernel
($\sigma$ = 100 ms), and peri-event time histograms (PETHs) were
generated by taking the average of the convolved spike trains across all
...
\DIFadd{To visualize NAc representations of task space within cue conditions,
normalized spike trains for all units were ordered by the location of
their maximum or minimum firing rate for a specified cue condition
(Figure \ref{fig:tiling}). To compare representations of task space
across cue conditions for a cue feature, the ordering of units derived
for one condition (e.g.\ light block) was then applied to the
normalized spike trains for the other condition }\DIFaddend (e.g.\DIFdelbegin \DIFdel{arm 1), would help shape their assignment of reward to that
...
and PFC inputs leads to the loss of outcome-predictive activity to discrete cues in the
NAc. Furthermore, non-discriminatory coding in
the NAc could hypothetically be correlated to over-generalization of situations,
in which the animal may not recognize it is in a different
situation and
perform actions that are inefficient or maladaptive for
reward procurement. }\DIFdelend \DIFaddbegin \DIFadd{\ sound
block). For control comparisons within cue conditions, half of the
trials for a condition were compared against the other half. To look
at the correlation of firing rates of all units within and across
various cue conditions, trials for each cue condition for a unit were
...
\DIFdelbegin \DIFdel{A fundamental problem faced by all reinforcement learning agents is
...
not only a RPE but also a trace of the preceding actions and }\DIFdelend \DIFaddbegin \DIFadd{To identify the responsivity of units to different cue features at the
time of a nosepoke into a reward receptacle, and subsequent reward
delivery, the same cue-responsive units from the cue-onset analyses
were analyzed at the time of nosepoke and outcome receipt using
identical analysis techniques (Figures \ref{fig:NP_examples},
...
\DIFadd{Given that some of our analyses compare firing rates across time,
particularly comparisons across blocks, we sought to exclude units
with unstable firing rates that would generate spurious results
reflecting a drift in firing rate over time unrelated to our task. To
do this we ran a Mann-Whitney U test comparing the cue-evoked firing
...
either block, leaving 344 units for further analyses. All analyses were completed in MATLAB R2015a, the code
is available on our public GitHub repository
(http:}\DIFaddend /\DIFdelbegin \DIFdel{or cues.
}\DIFdelend \DIFaddbegin \DIFadd{/github.com/vandermeerlab/papers), and the data can be accessed
through DataLad.
}

{\bf \DIFadd{Histology:}}

\DIFadd{Upon completion of the experiment, recording channels were gliosed by passing 10 $\mu A$ current for 10 seconds and waiting 5 days before euthanasia, except for rat R057 whose implant detached prematurely. Rats were anesthetized with 5\%
isoflurane, then asphyxiated with carbon dioxide. Transcardial
perfusions were performed, and brains were fixed and removed. Brains
were sliced in 50 $\mu m$ coronal sections and stained with
thionin. Slices were visualized under light microscopy, tetrode
...
\caption{\DIFaddFL{Histological verification of recording sites. Upon completion of
  experiments, brains were sectioned and tetrode placement was
  confirmed. \bsf{A}: Example section from R060 showing a recording site in the
  NAc core just dorsal to the anterior commissure (arrow). \bsf{B}:
  Schematic showing recording areas for all subjects.}}
