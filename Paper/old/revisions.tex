...
%DIF ADD vStrCueCoding_elife_revisions.tex   Wed Jun 27 22:00:20 2018
...
\cite{Schultz1992,Hollerman1998,Hassani2001,Cromwell2003,Setlow2003,Nicola2004,Roitman2005,Day2006,Roesch2009a,Saddoris2011,Goldstein2012,Lansink2012,Bissonette2013,McGinty2013,Atallah2014,Sugam2014,Cooch2015,West2016}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend ,
...
\cite{Ikemoto2007,McDannald2011,Hart2014,Hamid2016,Takahashi2016}. \DIFaddbegin \DIFadd{Similarly, in an economic framework, in order to maximize utility a rational agent selects the offer with the largest subjective value \mbox{%DIFAUXCMD
\cite{Samuelson1947}}\hspace{0pt}%DIFAUXCMD
. NAc has been implicated as containing representations of this domain-general subjective value \mbox{%DIFAUXCMD
\cite{Peters2009,Levy2012,Bartra2013,Sescousse2015}}\hspace{0pt}%DIFAUXCMD
, and having representations of an offer alongside its subjective value could be one way in which the two are linked.
}\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=0.6\textheight]{Fig 1 - Schematic neural.pdf}
%DIF > \includegraphics[width=\textwidth]{Fig 1 - Schematic neural alt.pdf}
\DIFaddendFL \caption{Schematic of potential coding strategies for cue identity (light, sound) and cue outcome (reward-available, reward-unavailable) employed by single units in the NAc across different units (A) and in time (B). \bsf{A}: Displayed are schematic PETHs illustrating putative responses to different cues under different hypotheses of how cue identity and outcome are coded. \DIFdelbeginFL \DIFdelFL{H1 (left }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Left }\DIFaddendFL panel\DIFdelbeginFL \DIFdelFL{)}\DIFdelendFL : Coding of cue identity is absent in the NAc. Top: Unit A encodes a motivationally relevant variable, such as expected outcome, similarly across other cue features, such as cue identity or physical location. Hypothetical plot is firing rate across time. L1+ (red) signifies a reward-available light cue, S1+ (navy blue) a reward-available sound cue, L2- (green) a reward-unavailable light cue, S2- (light blue) a reward-unavailable sound cue. Dashed line indicates onset of cue. Bottom: No units within the NAc discriminate their firing according to cue identity. \DIFdelbeginFL \DIFdelFL{H2 (middle }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Middle }\DIFaddendFL panel\DIFdelbeginFL \DIFdelFL{)}\DIFdelendFL : Coding of cue identity occurs \DIFdelbeginFL \DIFdelFL{independently }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{in a separate population }\DIFaddendFL of \DIFaddbeginFL \DIFaddFL{units from }\DIFaddendFL encoding of motivationally relevant variables such as expected outcome or subsequent vigor. Top: Same as H1, with unit A discriminating between reward-available and reward-unavailable cues. Bottom: Unit B discriminates firing across stimulus modalities, depicted here as firing to light cues but not sound cues. \DIFdelbeginFL \DIFdelFL{H3 (right }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Right }\DIFaddendFL panel\DIFdelbeginFL \DIFdelFL{)}\DIFdelendFL : Coding of cue identity \DIFdelbeginFL \DIFdelFL{is integrated }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{occurs in an overlapping population of cells }\DIFaddendFL with coding of other motivationally relevant variables. Hypothetical example demonstrating a unit that responds to outcome-predictive cues, but firing rate is also modulated by cue identity, firing most for the reward-available light cue. \bsf{B}: Displayed are schematic PETHs illustrating potential ways in which cue identity signals may persist over time. \DIFdelbeginFL \DIFdelFL{H1 (left }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Left }\DIFaddendFL panel\DIFdelbeginFL \DIFdelFL{)}\DIFdelendFL : Cue-onset triggers a transient response to a unit that codes for cue identity. Dashed lines indicate time of a behavioral or environmental event. 'Cue-ON' signifies onset of cue, 'NP' signifies when the rat holds a nosepoke at a reward receptacle, 'Out' signifies when the outcome is revealed, 'OFF' signifies when the cue turns off. \DIFdelbeginFL \DIFdelFL{H2 (middle
  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Middle and right }\DIFaddendFL panel\DIFdelbeginFL \DIFdelFL{)}\DIFdelendFL : Coding of cue identity persists \DIFaddbeginFL \DIFaddFL{at other time points, shown here }\DIFaddendFL during a nosepoke hold period until outcome is revealed. Coding can either be maintained by \DIFdelbeginFL \DIFdelFL{the same unit as during cue-onset (H2A) or by }\DIFdelendFL a sequence of units (\DIFdelbeginFL \DIFdelFL{H2B). H3 (right }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{middle }\DIFaddendFL panel) \DIFdelbeginFL \DIFdelFL{: Coding of cue identity persists after the outcome is received when the rat gets feedback about his decision,
  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{or }\DIFaddendFL by \DIFdelbeginFL \DIFdelFL{either }\DIFdelendFL the same unit as during cue-onset (\DIFdelbeginFL \DIFdelFL{H3A}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{right panel}\DIFaddendFL )\DIFdelbeginFL \DIFdelFL{or by a sequence of units (H3B)}\DIFdelendFL .\DIFdelbeginFL \DIFdelFL{The
...
\end{tabular}
\caption {Overview of recorded NAc units and their relationship to task variables at various time epochs. \DIFaddbeginFL \DIFaddFL{Percentage is relative to number of cue-modulated units}\DIFaddendFL } \label{tbl1} 
\end{table}
...
signal \DIFdelbegin \DIFdel{(Pennartz. , 2011). }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Pennartz2011}}\hspace{0pt}%DIFAUXCMD
. }\DIFaddend If this is the case, then the recruitment of NAc units
...
a nosepoke until the outcome was received, \DIFdelbegin \DIFdel{after }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend immediately after outcome receipt (\DIFdelbegin \DIFdel{H2 and H3 in }\DIFdelend Figure
\ref{fig:schematic}B). Units coding for cue identity \DIFdelbegin \DIFdel{showed partial
overlap with those }\DIFdelend \DIFaddbegin \DIFadd{were coded by an independent population than units }\DIFaddend coding for expected \DIFdelbegin \DIFdel{outcome (H3 in }\DIFdelend \DIFaddbegin \DIFadd{value (}\DIFaddend Figure
...
\DIFaddbegin \DIFadd{Viewed within the neuroeconomic framework of decision making it is possible that the NAc is encoding offers alongside their subjective value \mbox{%DIFAUXCMD
\cite{Samuelson1947}}\hspace{0pt}%DIFAUXCMD
. Traditionally, functional magnetic resonance imaging (fMRI) studies have found support for NAc representations of values of offers, specifically a domain-general subjective value signal that scales different rewards into a common currency so that they can be compared across different attributes, such as reward identity, effort required to obtain the reward, and temporal proximity to reward \mbox{%DIFAUXCMD
\cite{Peters2009,Levy2012,Bartra2013,Sescousse2015}}\hspace{0pt}%DIFAUXCMD
...
overlapping, }\DIFdelend \DIFaddbegin \DIFadd{independent }\DIFaddend populations of units encoding cue identity and expected
...
coding but not }\DIFdelend \DIFaddbegin \DIFadd{after removing variance for potential }\DIFaddend action value coding. Additionally, NAc signals have
...
\cite{Tabuchi2000,Pennartz2004,Lansink2008,Lansink2009,VanderMeer2011,Lansink2016,Sjulson2017}. \DIFaddbegin \DIFadd{Additionally our finding of a tiling of task space is in alignment with previous studies showing that }\DIFaddend NAc units can also signal progress through a sequence of cues and/or
actions \cite{Shidara1998,Mulder2004,Khamassi2008,Berke2009,Lansink2012,Atallah2014}\DIFaddbegin \DIFadd{, and may represent a temporally evolved state value signal \mbox{%DIFAUXCMD
\cite{Pennartz2011,Hamid2016}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Given
