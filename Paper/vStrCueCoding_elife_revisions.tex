...
\begin{figure} [t!]
  \caption{(Previous page.) \bsf{C}: Schematic pool of NAc units,
    ...
    expected by chance and the two populations are therefore
    independent from one another. A positive correlation (r $>$ 0)
...
of such cues. Importantly, this identity coding was maintained on
...
chance, Figure \ref{fig:schematic}A). Units that coded different cue
...
similar observations in the basal forebrain (CITE Tingley paper) and
...
independent overlapping population of units if there was no
significant correlation ($r = $ 0), and c) a joint overlapping
