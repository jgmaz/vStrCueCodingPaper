...
%                         if metadata.TrialInfo_block1.summary(jj,2) == metadata.TrialInfo_block1.summary(jj,3) % if correct response
 if metadata.TrialInfo_block1.nosepokeT(jj) ~= 0
             if metadata.TrialInfo_block1.nosepoke_length(jj) > .5
            ...
            nosepoke_to_click(trials_count) = 1;
                 end
        end_time(trials_count) = dataPoint.Nosepokes(trials_count)/1000 + nosepoke_to_click(trials_count);
        start_time(trials_count) = dataPoint.Nosepokes(trials_count)/1000 - nosepoke_to_click(trials_count);
        
        % now, count spikes between start and end
        these_spk = spk_t(spk_t > dataPoint.Nosepokes(trials_count)/1000 + .5 & spk_t < end_time(trials_count));
               
        % convert to firing rate and store
        firing_rate_np(trials_count) = length(these_spk) / nosepoke_to_click(trials_count);
if jj == 1
...
if metadata.TrialInfo_block2.nosepokeT(jj) ~= 0
        if metadata.TrialInfo_block2.nosepoke_length(jj) > .5
...
%                         if metadata.TrialInfo{1,1}.summary(jj,2) == metadata.TrialInfo{1,1}.summary(jj,3)
 if metadata.TrialInfo{1,1}.nosepokeT(jj) ~= 0
             if metadata.TrialInfo{1,1}.nosepoke_length(jj) > .5
...
if metadata.TrialInfo{1,2}.nosepokeT(jj) ~= 0
        if metadata.TrialInfo{1,2}.nosepoke_length(jj) > .5
