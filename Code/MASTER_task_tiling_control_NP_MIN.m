...
%                 case 0
    ...
    for iBin = 1:15001 %generate averaged responses
        ...
        PETHs.np_rew_1st_half{count}(iBin,:) = nanmean(PETH.Nosepoke.ALL.nosepoke_rew_PETH(iBin,half_1));
    PETHs.np_rew_2nd_half{count}(iBin,:) = nanmean(PETH.Nosepoke.ALL.nosepoke_rew_PETH(iBin,half_2));
        end
    end
    
    [minimum_rew_1st(count),min_location_rew_1st(count)] = min(PETHs.np_rew_1st_half{count}(cfg.start:cfg.end));
    [minimum_rew_2nd(count),min_location_rew_2nd(count)] = min(PETHs.np_rew_2nd_half{count}(cfg.start:cfg.end));
...
for jj = 1:length(mat_files_drift)  
    MINsortedPETH.rew_1st.unaltered(jj,:) = PETHs.np_rew_1st_half{MINsortPETH.rew_1st.index_sort(jj)}(cfg.start:cfg.end);
    MINsortedPETH.rew_2nd.rew_1st(jj,:) = PETHs.np_rew_2nd_half{MINsortPETH.rew_1st.index_sort(jj)}(cfg.start:cfg.end);
    MINsortedPETH.rew_2nd.unaltered(jj,:) = PETHs.np_rew_2nd_half{MINsortPETH.rew_2nd.index_sort(jj)}(cfg.start:cfg.end);
    disp(jj)
end
