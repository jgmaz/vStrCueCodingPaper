...
%                         if metadata.TrialInfo_block1.summary(jj,2) == metadata.TrialInfo_block1.summary(jj,3) % if correct response
 if metadata.TrialInfo_block1.nosepokeT(jj) ~= 0
             if metadata.TrialInfo_block1.nosepoke_length(jj) < 1
                 nosepoke_number = nosepoke_number + 1;
            continue
             elseif metadata.TrialInfo_block1.nosepoke_length(jj) < 1.5
            click_to_cue_off(trials_count) = metadata.TrialInfo_block1.nosepoke_length(jj);
        else
            click_to_cue_off(trials_count) = 1.5;
        end
        ...
        these_spk = spk_t(spk_t > dataPoint.Nosepokes(nosepoke_number)/1000 + 1 & spk_t < end_time(trials_count));
               
        % convert to firing rate and store
        firing_rate_np(trials_count) = length(these_spk) / (click_to_cue_off(trials_count) - 1);
...
if metadata.TrialInfo_block2.nosepokeT(jj) ~= 0
        if metadata.TrialInfo_block2.nosepoke_length(jj) < 1
            nosepoke_number = nosepoke_number + 1;
            continue
        elseif metadata.TrialInfo_block2.nosepoke_length(jj) < 1.5
...
%                         if metadata.TrialInfo{1,1}.summary(jj,2) == metadata.TrialInfo{1,1}.summary(jj,3)
 if metadata.TrialInfo{1,1}.nosepokeT(jj) ~= 0
             if metadata.TrialInfo{1,1}.nosepoke_length(jj) < 1
                 nosepoke_number = nosepoke_number + 1;
            continue
             elseif metadata.TrialInfo{1,1}.nosepoke_length(jj) < 1.5
...
if metadata.TrialInfo{1,2}.nosepokeT(jj) ~= 0
        if metadata.TrialInfo{1,2}.nosepoke_length(jj) < 1
            nosepoke_number = nosepoke_number + 1;
            continue
             elseif metadata.TrialInfo{1,2}.nosepoke_length(jj) < 1.5
