...
%                         if metadata.TrialInfo_block1.summary(jj,2) == metadata.TrialInfo_block1.summary(jj,3) % if correct response
 if metadata.TrialInfo_block1.nosepokeT(jj) ~= 0
        ...
        these_spk = spk_t(spk_t > dataPoint.Nosepokes(nosepoke_number)/1000 & spk_t < end_time(trials_count));
               
        % convert to firing rate and store
        firing_rate_np(trials_count) = length(these_spk) / click_to_cue_off(trials_count);
