...
%                         if metadata.TrialInfo_block1.summary(jj,2) == metadata.TrialInfo_block1.summary(jj,3) % if correct response
 if metadata.TrialInfo_block1.nosepokeT(jj) ~= 0
             if metadata.TrialInfo_block1.nosepoke_length(jj) < 1.5
                 nosepoke_number = nosepoke_number + 1;
            continue
             elseif metadata.TrialInfo_block1.nosepoke_length(jj) < 2
            click_to_cue_off(trials_count) = metadata.TrialInfo_block1.nosepoke_length(jj);
        else
            click_to_cue_off(trials_count) = 2;
        end
        end_time(trials_count) = dataPoint.Nosepokes(nosepoke_number)/1000 + click_to_cue_off(trials_count);
        
        % now, count spikes between start and end
        these_spk = spk_t(spk_t > dataPoint.Nosepokes(nosepoke_number)/1000 + 1.5 & spk_t < end_time(trials_count));
               
        % convert to firing rate and store
        firing_rate_np(trials_count) = length(these_spk) / (click_to_cue_off(trials_count) - 1.5);
...
if metadata.TrialInfo_block2.nosepokeT(jj) ~= 0
        if metadata.TrialInfo_block2.nosepoke_length(jj) < 1.5
            nosepoke_number = nosepoke_number + 1;
            continue
        elseif metadata.TrialInfo_block2.nosepoke_length(jj) < 2
            click_to_cue_off(trials_count) = metadata.TrialInfo_block2.nosepoke_length(jj);
        else
            click_to_cue_off(trials_count) = 2;
        end
        end_time(trials_count) = dataPoint.Nosepokes(nosepoke_number)/1000 + click_to_cue_off(trials_count);
...
%                         if metadata.TrialInfo{1,1}.summary(jj,2) == metadata.TrialInfo{1,1}.summary(jj,3)
 if metadata.TrialInfo{1,1}.nosepokeT(jj) ~= 0
             if metadata.TrialInfo{1,1}.nosepoke_length(jj) < 1.5
                 nosepoke_number = nosepoke_number + 1;
            continue
             elseif metadata.TrialInfo{1,1}.nosepoke_length(jj) < 2
            click_to_cue_off(trials_count) = metadata.TrialInfo{1,1}.nosepoke_length(jj);
        else
            click_to_cue_off(trials_count) = 2;
        end
        end_time(trials_count) = dataPoint.Nosepokes(nosepoke_number)/1000 + click_to_cue_off(trials_count);
...
if metadata.TrialInfo{1,2}.nosepokeT(jj) ~= 0
        if metadata.TrialInfo{1,2}.nosepoke_length(jj) < 1.5
            nosepoke_number = nosepoke_number + 1;
            continue
             elseif metadata.TrialInfo{1,2}.nosepoke_length(jj) < 2
            click_to_cue_off(trials_count) = metadata.TrialInfo{1,2}.nosepoke_length(jj);
        else
            click_to_cue_off(trials_count) = 2;
        end
        end_time(trials_count) = dataPoint.Nosepokes(nosepoke_number)/1000 + click_to_cue_off(trials_count);
...
GLM_matrices.dev_percent.cat_combined = cat(1,GLM_matrices.dev_percent.SPN_Inc,SPN_Dec_inverse,HFN_Dec_inverse)';
% cell 84 acts weird (unusually high values)
map = [233/255,163/255,201/255
    247/255,247/255,247/255
    161/255,215/255,106/255];

rColorMap = [linspace(233/255, 255/255, 194),linspace(255/255, 161/255, 32)];
    gColorMap = [linspace(163/255, 255/255, 194),linspace(255/255, 215/255, 32)];
    bColorMap = [linspace(201/255, 255/255, 194),linspace(255/255, 106/255, 32)];
