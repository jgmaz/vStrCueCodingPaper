...
%     end
    ...
    if RANK.two.Trial > 975 || RANK.two.Trial < 26
        if TESTS.WSR.Task.Trial_b4_vs_Trial < .01
            ...
            for iShuff = 1:100    
                spk_t_SHUFF = [];
                shuff_ISI = datasample(ISI,length(ISI),'Replace',false);  
spk_t_SHUFF(1) = spk_t(1);
for iSpk = 2:length(spk_t);
    spk_t_SHUFF(iSpk) = spk_t_SHUFF(iSpk-1) + shuff_ISI(iSpk-1);
end
    ...
    these_spk = spk_t_SHUFF(spk_t_SHUFF > start_time(trials_count) & spk_t_SHUFF < end_time(trials_count));
...
save(cat(2,'E:\Jimmie\Jimmie\Analysis\2018-06-08-GLM_cueon_',num2str(iTime),'-SHUFF.mat'),'mdl')
clearvars -except iTime
    end
%%
    for iTime = -.5:.1:.5
        load(cat(2,'E:\Jimmie\Jimmie\Analysis\2018-06-08-GLM_cueon_',num2str(iTime),'-SHUFF.mat'))
...
for kk = 1:length(dir('*.mat'))
    ...
    for iShuff = 1:100
       ...
       if isfield(mdl{kk},cat(2,'shuff_',num2str(iShuff))) == 1
...
save(cat(2,'E:\Jimmie\Jimmie\Analysis\2018-06-09-GLM_cueon_',num2str(iTime),'-SHUFF.mat'),'mdl','ALL_matrix','block_drift','GLM_matrices','summary_var')
clearvars -except iTime
    end
