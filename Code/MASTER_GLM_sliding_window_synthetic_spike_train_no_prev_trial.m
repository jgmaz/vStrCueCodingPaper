...
%     end
    ...
    these_spk = synth.spk_t{kk}(synth.spk_t{kk} > start_time(trials_count) & synth.spk_t{kk} < end_time(trials_count));
...
%                         end
            ...
            if sum(dataset(:,7)) < 10
                ...
                %%
                ds = mat2dataset(dataset(:,2:end),'VarNames',{'Outcome','Modality','Location','Approach','Latency','Trial','FiringRate'});
...
save(cat(2,'E:\Jimmie\Jimmie\Analysis\2018-06-02-GLM_cueon_',num2str(iTime),'-SYNTH_full_np_no_prev_trial.mat'),'mdl','ALL_matrix','block_drift','GLM_matrices','summary_var')
clearvars -except iTime iRound synth
    end
end
%%
