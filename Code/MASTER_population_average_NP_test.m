...
% load_vars = find(ALL_matrix(:,1) == 1 & ALL_matrix_NP(:,1) == 1);
load_vars = find(ALL_matrix(:,1) == 0 & ALL_matrix_NP(:,1) == 1);
...
for jj = 1:15001 %generate averaged responses
    if jj <= length(rule_encoding.RAW.pref.ALL.MEAN)
        rule_encoding.RAW.pref.AVG.MEAN(jj,1) = nanmean(rule_encoding.RAW.pref.ALL.MEAN(jj,:));
        rule_encoding.RAW.pref.AVG.SEM(jj,1) = nanstd(rule_encoding.RAW.pref.ALL.MEAN(jj,:)/sqrt(numel(rule_encoding.RAW.pref.ALL.MEAN(jj,:))-sum(isnan(rule_encoding.RAW.pref.ALL.MEAN(jj,:)))));
    end
    
    if jj <= length(rule_encoding.RAW.nonpref.ALL.MEAN)
        rule_encoding.RAW.nonpref.AVG.MEAN(jj,1) = nanmean(rule_encoding.RAW.nonpref.ALL.MEAN(jj,:));
        rule_encoding.RAW.nonpref.AVG.SEM(jj,1) = nanstd(rule_encoding.RAW.nonpref.ALL.MEAN(jj,:)/sqrt(numel(rule_encoding.RAW.nonpref.ALL.MEAN(jj,:))-sum(isnan(rule_encoding.RAW.nonpref.ALL.MEAN(jj,:)))));
    end
    
    if jj <= length(rule_encoding.zscore.pref.ALL.MEAN)
        rule_encoding.zscore.pref.AVG.MEAN(jj,1) = nanmean(rule_encoding.zscore.pref.ALL.MEAN(jj,:));
        rule_encoding.zscore.pref.AVG.SEM(jj,1) = nanstd(rule_encoding.zscore.pref.ALL.MEAN(jj,:)/sqrt(numel(rule_encoding.zscore.pref.ALL.MEAN(jj,:))-sum(isnan(rule_encoding.zscore.pref.ALL.MEAN(jj,:)))));
    end
    
    if jj <= length(rule_encoding.zscore.nonpref.ALL.MEAN)
        rule_encoding.zscore.nonpref.AVG.MEAN(jj,1) = nanmean(rule_encoding.zscore.nonpref.ALL.MEAN(jj,:));
        rule_encoding.zscore.nonpref.AVG.SEM(jj,1) = nanstd(rule_encoding.zscore.nonpref.ALL.MEAN(jj,:)/sqrt(numel(rule_encoding.zscore.nonpref.ALL.MEAN(jj,:))-sum(isnan(rule_encoding.zscore.nonpref.ALL.MEAN(jj,:)))));
    end
    
    if jj <= length(rule_encoding.zscore.pref.INC.MEAN)
        rule_encoding.zscore.pref.INC.AVG.MEAN(jj,1) = nanmean(rule_encoding.zscore.pref.INC.MEAN(jj,:));
        rule_encoding.zscore.pref.INC.AVG.SEM(jj,1) = nanstd(rule_encoding.zscore.pref.INC.MEAN(jj,:)/sqrt(numel(rule_encoding.zscore.pref.INC.MEAN(jj,:))-sum(isnan(rule_encoding.zscore.pref.INC.MEAN(jj,:)))));
    end
    
    if jj <= length(rule_encoding.zscore.nonpref.INC.MEAN)
        rule_encoding.zscore.nonpref.INC.AVG.MEAN(jj,1) = nanmean(rule_encoding.zscore.nonpref.INC.MEAN(jj,:));
        rule_encoding.zscore.nonpref.INC.AVG.SEM(jj,1) = nanstd(rule_encoding.zscore.nonpref.INC.MEAN(jj,:)/sqrt(numel(rule_encoding.zscore.nonpref.INC.MEAN(jj,:))-sum(isnan(rule_encoding.zscore.nonpref.INC.MEAN(jj,:)))));
    end
    
    if jj <= length(rule_encoding.zscore.pref.DEC.MEAN)
        rule_encoding.zscore.pref.DEC.AVG.MEAN(jj,1) = nanmean(rule_encoding.zscore.pref.DEC.MEAN(jj,:));
        rule_encoding.zscore.pref.DEC.AVG.SEM(jj,1) = nanstd(rule_encoding.zscore.pref.DEC.MEAN(jj,:)/sqrt(numel(rule_encoding.zscore.pref.DEC.MEAN(jj,:))-sum(isnan(rule_encoding.zscore.pref.DEC.MEAN(jj,:)))));
    end
    
    if jj <= length(rule_encoding.zscore.nonpref.DEC.MEAN)
        rule_encoding.zscore.nonpref.DEC.AVG.MEAN(jj,1) = nanmean(rule_encoding.zscore.nonpref.DEC.MEAN(jj,:));
        rule_encoding.zscore.nonpref.DEC.AVG.SEM(jj,1) = nanstd(rule_encoding.zscore.nonpref.DEC.MEAN(jj,:)/sqrt(numel(rule_encoding.zscore.nonpref.DEC.MEAN(jj,:))-sum(isnan(rule_encoding.zscore.nonpref.DEC.MEAN(jj,:)))));
    end
end

%% average for location
% load_vars = find(ALL_matrix(:,2) == 1 & ALL_matrix_NP(:,2) == 1);
load_vars = find(ALL_matrix(:,2) == 0 & ALL_matrix_NP(:,2) == 1);
...
for jj = 1:15001 %generate averaged responses
    if jj <= length(location_encoding.RAW.one.ALL.MEAN)
        location_encoding.RAW.one.AVG.MEAN(jj,1) = nanmean(location_encoding.RAW.one.ALL.MEAN(jj,:));
        location_encoding.RAW.one.AVG.SEM(jj,1) = nanstd(location_encoding.RAW.one.ALL.MEAN(jj,:)/sqrt(numel(location_encoding.RAW.one.ALL.MEAN(jj,:))-sum(isnan(location_encoding.RAW.one.ALL.MEAN(jj,:)))));
    end
    
    if jj <= length(location_encoding.zscore.one.ALL.MEAN)
        location_encoding.zscore.one.AVG.MEAN(jj,1) = nanmean(location_encoding.zscore.one.ALL.MEAN(jj,:));
        location_encoding.zscore.one.AVG.SEM(jj,1) = nanstd(location_encoding.zscore.one.ALL.MEAN(jj,:)/sqrt(numel(location_encoding.zscore.one.ALL.MEAN(jj,:))-sum(isnan(location_encoding.zscore.one.ALL.MEAN(jj,:)))));
    end
    
    if jj <= length(location_encoding.zscore.one.INC.MEAN)
        location_encoding.zscore.one.INC.AVG.MEAN(jj,1) = nanmean(location_encoding.zscore.one.INC.MEAN(jj,:));
        location_encoding.zscore.one.INC.AVG.SEM(jj,1) = nanstd(location_encoding.zscore.one.INC.MEAN(jj,:)/sqrt(numel(location_encoding.zscore.one.INC.MEAN(jj,:))-sum(isnan(location_encoding.zscore.one.INC.MEAN(jj,:)))));
    end
    
    if jj <= length(location_encoding.zscore.one.DEC.MEAN)
        location_encoding.zscore.one.DEC.AVG.MEAN(jj,1) = nanmean(location_encoding.zscore.one.DEC.MEAN(jj,:));
        location_encoding.zscore.one.DEC.AVG.SEM(jj,1) = nanstd(location_encoding.zscore.one.DEC.MEAN(jj,:)/sqrt(numel(location_encoding.zscore.one.DEC.MEAN(jj,:))-sum(isnan(location_encoding.zscore.one.DEC.MEAN(jj,:)))));
    end
    
    if jj <= length(location_encoding.RAW.two.ALL.MEAN)
        location_encoding.RAW.two.AVG.MEAN(jj,1) = nanmean(location_encoding.RAW.two.ALL.MEAN(jj,:));
        location_encoding.RAW.two.AVG.SEM(jj,1) = nanstd(location_encoding.RAW.two.ALL.MEAN(jj,:)/sqrt(numel(location_encoding.RAW.two.ALL.MEAN(jj,:))-sum(isnan(location_encoding.RAW.two.ALL.MEAN(jj,:)))));
    end
    
    if jj <= length(location_encoding.zscore.two.ALL.MEAN)
        location_encoding.zscore.two.AVG.MEAN(jj,1) = nanmean(location_encoding.zscore.two.ALL.MEAN(jj,:));
        location_encoding.zscore.two.AVG.SEM(jj,1) = nanstd(location_encoding.zscore.two.ALL.MEAN(jj,:)/sqrt(numel(location_encoding.zscore.two.ALL.MEAN(jj,:))-sum(isnan(location_encoding.zscore.two.ALL.MEAN(jj,:)))));
    end
    
    if jj <= length(location_encoding.zscore.two.INC.MEAN)
        location_encoding.zscore.two.INC.AVG.MEAN(jj,1) = nanmean(location_encoding.zscore.two.INC.MEAN(jj,:));
        location_encoding.zscore.two.INC.AVG.SEM(jj,1) = nanstd(location_encoding.zscore.two.INC.MEAN(jj,:)/sqrt(numel(location_encoding.zscore.two.INC.MEAN(jj,:))-sum(isnan(location_encoding.zscore.two.INC.MEAN(jj,:)))));
    end
    
    if jj <= length(location_encoding.zscore.two.DEC.MEAN)
        location_encoding.zscore.two.DEC.AVG.MEAN(jj,1) = nanmean(location_encoding.zscore.two.DEC.MEAN(jj,:));
        location_encoding.zscore.two.DEC.AVG.SEM(jj,1) = nanstd(location_encoding.zscore.two.DEC.MEAN(jj,:)/sqrt(numel(location_encoding.zscore.two.DEC.MEAN(jj,:))-sum(isnan(location_encoding.zscore.two.DEC.MEAN(jj,:)))));
    end
    
    if jj <= length(location_encoding.RAW.three.ALL.MEAN)
        location_encoding.RAW.three.AVG.MEAN(jj,1) = nanmean(location_encoding.RAW.three.ALL.MEAN(jj,:));
        location_encoding.RAW.three.AVG.SEM(jj,1) = nanstd(location_encoding.RAW.three.ALL.MEAN(jj,:)/sqrt(numel(location_encoding.RAW.three.ALL.MEAN(jj,:))-sum(isnan(location_encoding.RAW.three.ALL.MEAN(jj,:)))));
    end
    
    if jj <= length(location_encoding.zscore.three.ALL.MEAN)
        location_encoding.zscore.three.AVG.MEAN(jj,1) = nanmean(location_encoding.zscore.three.ALL.MEAN(jj,:));
        location_encoding.zscore.three.AVG.SEM(jj,1) = nanstd(location_encoding.zscore.three.ALL.MEAN(jj,:)/sqrt(numel(location_encoding.zscore.three.ALL.MEAN(jj,:))-sum(isnan(location_encoding.zscore.three.ALL.MEAN(jj,:)))));
    end
    
    if jj <= length(location_encoding.zscore.three.INC.MEAN)
        location_encoding.zscore.three.INC.AVG.MEAN(jj,1) = nanmean(location_encoding.zscore.three.INC.MEAN(jj,:));
        location_encoding.zscore.three.INC.AVG.SEM(jj,1) = nanstd(location_encoding.zscore.three.INC.MEAN(jj,:)/sqrt(numel(location_encoding.zscore.three.INC.MEAN(jj,:))-sum(isnan(location_encoding.zscore.three.INC.MEAN(jj,:)))));
    end
    
    if jj <= length(location_encoding.zscore.three.DEC.MEAN)
        location_encoding.zscore.three.DEC.AVG.MEAN(jj,1) = nanmean(location_encoding.zscore.three.DEC.MEAN(jj,:));
        location_encoding.zscore.three.DEC.AVG.SEM(jj,1) = nanstd(location_encoding.zscore.three.DEC.MEAN(jj,:)/sqrt(numel(location_encoding.zscore.three.DEC.MEAN(jj,:))-sum(isnan(location_encoding.zscore.three.DEC.MEAN(jj,:)))));
    end
    
    if jj <= length(location_encoding.RAW.four.ALL.MEAN)
        location_encoding.RAW.four.AVG.MEAN(jj,1) = nanmean(location_encoding.RAW.four.ALL.MEAN(jj,:));
        location_encoding.RAW.four.AVG.SEM(jj,1) = nanstd(location_encoding.RAW.four.ALL.MEAN(jj,:)/sqrt(numel(location_encoding.RAW.four.ALL.MEAN(jj,:))-sum(isnan(location_encoding.RAW.four.ALL.MEAN(jj,:)))));
    end
    
    if jj <= length(location_encoding.zscore.four.ALL.MEAN)
        location_encoding.zscore.four.AVG.MEAN(jj,1) = nanmean(location_encoding.zscore.four.ALL.MEAN(jj,:));
        location_encoding.zscore.four.AVG.SEM(jj,1) = nanstd(location_encoding.zscore.four.ALL.MEAN(jj,:)/sqrt(numel(location_encoding.zscore.four.ALL.MEAN(jj,:))-sum(isnan(location_encoding.zscore.four.ALL.MEAN(jj,:)))));
    end
    
    if jj <= length(location_encoding.zscore.four.INC.MEAN)
        location_encoding.zscore.four.INC.AVG.MEAN(jj,1) = nanmean(location_encoding.zscore.four.INC.MEAN(jj,:));
        location_encoding.zscore.four.INC.AVG.SEM(jj,1) = nanstd(location_encoding.zscore.four.INC.MEAN(jj,:)/sqrt(numel(location_encoding.zscore.four.INC.MEAN(jj,:))-sum(isnan(location_encoding.zscore.four.INC.MEAN(jj,:)))));
    end
    
    if jj <= length(location_encoding.zscore.four.DEC.MEAN)
        location_encoding.zscore.four.DEC.AVG.MEAN(jj,1) = nanmean(location_encoding.zscore.four.DEC.MEAN(jj,:));
        location_encoding.zscore.four.DEC.AVG.SEM(jj,1) = nanstd(location_encoding.zscore.four.DEC.MEAN(jj,:)/sqrt(numel(location_encoding.zscore.four.DEC.MEAN(jj,:))-sum(isnan(location_encoding.zscore.four.DEC.MEAN(jj,:)))));
    end
    
end

%% outcome
% load_vars = find(ALL_matrix(:,3) == 1 & ALL_matrix_NP(:,3) == 1);
load_vars = find(ALL_matrix(:,3) == 0 & ALL_matrix_NP(:,3) == 1);
...
for jj = 1:15001 %generate averaged responses
    if jj <= length(outcome_encoding.RAW.pref.ALL.MEAN)
        outcome_encoding.RAW.pref.AVG.MEAN(jj,1) = nanmean(outcome_encoding.RAW.pref.ALL.MEAN(jj,:));
        outcome_encoding.RAW.pref.AVG.SEM(jj,1) = nanstd(outcome_encoding.RAW.pref.ALL.MEAN(jj,:)/sqrt(numel(outcome_encoding.RAW.pref.ALL.MEAN(jj,:))-sum(isnan(outcome_encoding.RAW.pref.ALL.MEAN(jj,:)))));
    end
    
    if jj <= length(outcome_encoding.RAW.nonpref.ALL.MEAN)
        outcome_encoding.RAW.nonpref.AVG.MEAN(jj,1) = nanmean(outcome_encoding.RAW.nonpref.ALL.MEAN(jj,:));
        outcome_encoding.RAW.nonpref.AVG.SEM(jj,1) = nanstd(outcome_encoding.RAW.nonpref.ALL.MEAN(jj,:)/sqrt(numel(outcome_encoding.RAW.nonpref.ALL.MEAN(jj,:))-sum(isnan(outcome_encoding.RAW.nonpref.ALL.MEAN(jj,:)))));
    end
    
    if jj <= length(outcome_encoding.zscore.pref.ALL.MEAN)
        outcome_encoding.zscore.pref.AVG.MEAN(jj,1) = nanmean(outcome_encoding.zscore.pref.ALL.MEAN(jj,:));
        outcome_encoding.zscore.pref.AVG.SEM(jj,1) = nanstd(outcome_encoding.zscore.pref.ALL.MEAN(jj,:)/sqrt(numel(outcome_encoding.zscore.pref.ALL.MEAN(jj,:))-sum(isnan(outcome_encoding.zscore.pref.ALL.MEAN(jj,:)))));
    end
    
    if jj <= length(outcome_encoding.zscore.nonpref.ALL.MEAN)
        outcome_encoding.zscore.nonpref.AVG.MEAN(jj,1) = nanmean(outcome_encoding.zscore.nonpref.ALL.MEAN(jj,:));
        outcome_encoding.zscore.nonpref.AVG.SEM(jj,1) = nanstd(outcome_encoding.zscore.nonpref.ALL.MEAN(jj,:)/sqrt(numel(outcome_encoding.zscore.nonpref.ALL.MEAN(jj,:))-sum(isnan(outcome_encoding.zscore.nonpref.ALL.MEAN(jj,:)))));
    end
    
    if jj <= length(outcome_encoding.zscore.pref.INC.MEAN)
        outcome_encoding.zscore.pref.INC.AVG.MEAN(jj,1) = nanmean(outcome_encoding.zscore.pref.INC.MEAN(jj,:));
        outcome_encoding.zscore.pref.INC.AVG.SEM(jj,1) = nanstd(outcome_encoding.zscore.pref.INC.MEAN(jj,:)/sqrt(numel(outcome_encoding.zscore.pref.INC.MEAN(jj,:))-sum(isnan(outcome_encoding.zscore.pref.INC.MEAN(jj,:)))));
    end
    
    if jj <= length(outcome_encoding.zscore.nonpref.INC.MEAN)
        outcome_encoding.zscore.nonpref.INC.AVG.MEAN(jj,1) = nanmean(outcome_encoding.zscore.nonpref.INC.MEAN(jj,:));
        outcome_encoding.zscore.nonpref.INC.AVG.SEM(jj,1) = nanstd(outcome_encoding.zscore.nonpref.INC.MEAN(jj,:)/sqrt(numel(outcome_encoding.zscore.nonpref.INC.MEAN(jj,:))-sum(isnan(outcome_encoding.zscore.nonpref.INC.MEAN(jj,:)))));
    end
    
    if jj <= length(outcome_encoding.zscore.pref.DEC.MEAN)
        outcome_encoding.zscore.pref.DEC.AVG.MEAN(jj,1) = nanmean(outcome_encoding.zscore.pref.DEC.MEAN(jj,:));
        outcome_encoding.zscore.pref.DEC.AVG.SEM(jj,1) = nanstd(outcome_encoding.zscore.pref.DEC.MEAN(jj,:)/sqrt(numel(outcome_encoding.zscore.pref.DEC.MEAN(jj,:))-sum(isnan(outcome_encoding.zscore.pref.DEC.MEAN(jj,:)))));
    end
    
    if jj <= length(outcome_encoding.zscore.nonpref.DEC.MEAN)
        outcome_encoding.zscore.nonpref.DEC.AVG.MEAN(jj,1) = nanmean(outcome_encoding.zscore.nonpref.DEC.MEAN(jj,:));
        outcome_encoding.zscore.nonpref.DEC.AVG.SEM(jj,1) = nanstd(outcome_encoding.zscore.nonpref.DEC.MEAN(jj,:)/sqrt(numel(outcome_encoding.zscore.nonpref.DEC.MEAN(jj,:))-sum(isnan(outcome_encoding.zscore.nonpref.DEC.MEAN(jj,:)))));
    end
end
