...
for iEpoch = 1:length(Epochs);
    ...
    for iPred = 1:length(Predictors);
        GLM_window.Table.(Epochs{iEpoch}).(Predictors{iPred}).Data = GLM_window.DATA.(Epochs{iEpoch}).prop(iPred+1,:);
        GLM_window.Table.(Epochs{iEpoch}).(Predictors{iPred}).ShuffMEAN = mean(GLM_window.SHUFF.(Epochs{iEpoch}).prop.ALL.(Predictors{iPred}));
        GLM_window.Table.(Epochs{iEpoch}).(Predictors{iPred}).ShuffSTD = std(GLM_window.SHUFF.(Epochs{iEpoch}).prop.ALL.(Predictors{iPred}));
        for iTime = 1:length(GLM_window.Table.(Epochs{iEpoch}).(Predictors{iPred}).Data)
            GLM_window.Table.(Epochs{iEpoch}).(Predictors{iPred}).Zscore(iTime) = (GLM_window.Table.(Epochs{iEpoch}).(Predictors{iPred}).Data(iTime) - GLM_window.Table.(Epochs{iEpoch}).(Predictors{iPred}).ShuffMEAN(iTime)) / GLM_window.Table.(Epochs{iEpoch}).(Predictors{iPred}).ShuffSTD(iTime);
            if GLM_window.Table.(Epochs{iEpoch}).(Predictors{iPred}).Zscore(iTime) > 1.96
                GLM_window.Table.(Epochs{iEpoch}).(Predictors{iPred}).Zscore_recode(iTime) = 1;
            else
                GLM_window.Table.(Epochs{iEpoch}).(Predictors{iPred}).Zscore_recode(iTime) = 0;
            end
        end
    end
end
...
% GLM_window.SHUFF.(Epoch{iEpoch}).Rsquared.ZSTD.(predictor_list{iList}) = nanstd(GLM_window.SHUFF.(Epoch{iEpoch}).Rsquared.ALL.(predictor_list{iList})) *1.96;
end
end
...
% hold on
% shadedErrorBar(-.5:.1:.5,GLM_window.SHUFF.cueon.prop.MEAN.Location,GLM_window.SHUFF.cueon.prop.ZSTD.Location,'--r',1);
% shadedErrorBar(-.5:.1:.5,GLM_window.SHUFF.cueon.prop.MEAN.Outcome,GLM_window.SHUFF.cueon.prop.ZSTD.Outcome,'--g',1);
% plot(-.5:.1:.5,GLM_window.DATA.cueon.prop(2,:),'-ob','MarkerFaceColor','b','MarkerSize',4)
% plot(-.5:.1:.5,GLM_window.DATA.cueon.prop(3,:),'-or','MarkerFaceColor','r','MarkerSize',4)
% plot(-.5:.1:.5,GLM_window.DATA.cueon.prop(4,:),'-og','MarkerFaceColor','g','MarkerSize',4)
% 
% for iTime = 1:length(GLM_window.Table.cueon.Outcome.Zscore_recode)
%     if GLM_window.Table.cueon.Modality.Zscore_recode(iTime) == 1
%         plot([-.65+(iTime*.1) -.55+(iTime*.1)],[.03 .03], '-k', 'LineWidth',2,'color','b')
...
% shadedErrorBar(-.5:.1:.5,GLM_window.DATA.cueon.Rsquared.MEAN(3,:),GLM_window.DATA.cueon.Rsquared.SEM(3,:),'-g',1);
...
% shadedErrorBar(-.5:.1:.5,GLM_window.SHUFF.cueon.Rsquared.MEAN.Outcome,GLM_window.SHUFF.cueon.Rsquared.SEM.Outcome,'--g',1);
