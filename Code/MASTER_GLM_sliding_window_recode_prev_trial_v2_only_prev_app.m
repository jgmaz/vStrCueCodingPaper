        ...
        if TESTS.WSR.Task.Trial_b4_vs_Trial < .01
            ...
            switch new_v_old
                case 0
                    ...
                    for jj = 2:b1_length                      
                        if metadata.TrialInfo_block1.approached(jj-1) == 1
...
dataset(trials_count,1) = metadata.TrialInfo_block1.rewarded(jj-1);
                            end
...
%                         end
                    ...
                    for jj = 2:b2_length
                        if metadata.TrialInfo_block2.approached(jj-1) == 1
...
dataset(trials_count,1) = metadata.TrialInfo_block2.rewarded(jj-1);
                            end
...
%                         end
                ...
                case 1
                    ...
                    b1_length = length(FRATE.Cue.Trial_firing_rate_block1);
                    for jj = 2:b1_length
                        if metadata.TrialInfo{1,1}.approached(jj-1) == 1
...
%                         if metadata.TrialInfo{1,1}.summary(jj,2) == metadata.TrialInfo{1,1}.summary(jj,3)
                            if jj == 1
                                dataset(trials_count,1) = NaN; %prev trial
                                 ...
                                 dataset(trials_count,1) = metadata.TrialInfo{1,1}.rewarded(jj-1);
...
%                         end
                    ...
                    for jj = 2:b2_length
                        if metadata.TrialInfo{1,2}.approached(jj-1) == 1
...
dataset(trials_count,1) = metadata.TrialInfo{1,2}.rewarded(jj-1);
                            end
...
save(cat(2,'E:\Jimmie\Jimmie\Analysis\2018-06-11-GLM_cueon_',num2str(iTime),'-recodePrevTrial_v2_prev_App_only','.mat'),'mdl','ALL_matrix','block_drift','GLM_matrices','summary_var')
